plot_posterior_vs_prior_parameters <- function(input_df,
  ...
  if (method == "commensurate_power_prior") {
    warning(
            paste0("Not implemented for the commensurate",
              " power prior, refer to the corresponding", " table instead."))
    return()
  }
  ...
  source_treatment_effect_estimate <-
    unique(results_metrics_df$source_treatment_effect_estimate)[1]
  ...
  posterior_parameters_df <- get_parameters(results_metrics_df[
    ,
  ...
  for (key in names(methods_dict[[method]])) {
    ...
    if (method != "commensurate_power_prior" &&
          !key %in% colnames(posterior_parameters_df)) {
      next
    }
    ...
    for (j in seq(n_params_loop)) {
      if ((is.null(nrow(other_parameters)) || nrow(other_parameters) == 0) ||
        ...
        # matching_filter <- apply(prior_parameters_df[,-k], 1, function(row)
        # compare_ignore_na(row, other_parameters[j,]))

        matching_filter <- apply(
          prior_parameters_df[, -k], 1,
      ...
      posterior_keys <- posterior_colnames[!grepl(
        "^conf_int",
        posterior_colnames
      )]

      for (posterior_key in posterior_keys) {
        # Combine all necessary data based on the drift values into a single
        # data frame
        combined_data <- do.call(rbind, lapply(
          mandatory_drift_values,
          function(drift) {
            filter_df <-
              abs(results_metrics_subdf$drift - drift) < .Machine$double.eps^0.5
            ...
            parameters_labels <-
              unname(sapply(seq_len(nrow(filtered_results_metrics_df)),
                            function(i) {
                              process_method_parameters_label(
                                filtered_results_metrics_df[i, ],
                                methods_labels,
                                method_name = FALSE
                              )
                            }))
            parameters_labels <- setNames(
              object = parameters_labels, nm =
                filtered_results_metrics_df$parameters
            )

            x <- as.numeric(prior_parameters[[key]])
            y <- as.numeric(posterior_parameters[[posterior_key]])
            conf_int_lower <-
              unlist(posterior_parameters[[paste0("conf_int_lower_",
                                                  posterior_key)]])
            conf_int_upper <-
              unlist(posterior_parameters[[paste0("conf_int_upper_",
                                                  posterior_key)]])
        ...
        cap_size <- x_width *
          relative_error_cap_width # Adjust relative_error_cap_width to control the relative cap size

        # Create the plot
        plt <- ggplot(combined_data, aes(
          x = x, y = y, ymin = conf_int_lower,
          ymax = conf_int_upper, color = as.factor(drift)
        )) +
          geom_errorbar(width = cap_size, position = position_dodge(
            width =
              cap_size / 2
          )) +
          geom_point(size = markersize, position = position_dodge(
            width =
              cap_size / 2
          )) +
          scale_color_manual(
            values = color_map, name = NULL, labels =
              treatment_effects_names
          ) +
          ggplot2::labs(
            title = plot_title,
            x = TeX(sprintf(
              "%s %s", "Prior",
              methods_dict[[method]][[key]]$parameter_notation
            )),
            y = TeX(sprintf(
              "%s %s", "Posterior",
              methods_dict[[method]][[posterior_key]]$parameter_notation
            )),
        ...
        figure_name <- format_filename(
          filename = figure_name, case_study =
            case_study, target_to_source_std_ratio = target_to_source_std_ratio,
          source_denominator_change_factor = source_denominator_change_factor
        )
        ...
        if (file.exists(paste0(file_path, ".pdf")) &&
              file.exists(paste0(file_path, ".png")) && remake_figures ==
              FALSE) {
          next
        }
...
#'
#' @description This function plots the posterior parameters against the drift
#'   for a given method, case study, sample size, and control
#'   drift option.
#'
...
#' @param control_drift A logical value indicating whether to filter the data
#'   based on control drift.
...
plot_posterior_parameters_vs_drift <- function(results_metrics_df,
                                               ...
                                               source_denominator_change_factor,
                                               target_to_source_std_ratio) {
  # Filter the data frame based on method, case_study, and
  # target_sample_size_per_arm
  ...
  posterior_parameters_names <- colnames(get_parameters(results_metrics_df[
    ,
    "posterior_parameters"
  ]))
  ...
  for (key in names(methods_dict[[method]])) {
    reference_values_all_parameters[[key]] <-
      methods_dict[[method]][[key]][["important_values"]]
  }
  ...
  if (method == "commensurate_power_prior") {
    parameters_combinations <- get_parameters(results_metrics_df[
      ,
      "parameters"
    ])
  }
  ...
  for (key in parameters_names) {
    ...
    if (method != "commensurate_power_prior" &&
          !key %in% posterior_parameters_names) {
      next
    }

    # Get the important prior values for which we will make plots (one per
    # important value)
    if (is.null(methods_dict[[method]][[key]][["important_values"]])) {
      important_parameters_values <- methods_dict[[method]][[key]][["range"]]
    } else {
    ...
    if (!(method == "commensurate_power_prior")) {
      # Other prior parameters can take different values, so we filter based on
      ...
      prior_parameters_df <- get_parameters(results_metrics_df[, "parameters"])
      boolean_filter <- create_boolean_filter(prior_parameters_df,
        filter_conditions,
        exclude_key = key
      )
    ...
    posterior_parameters_df <- get_parameters(results_metrics_subdf[
      ,
      "posterior_parameters"
    ])
    ...
    parameters_labels <- unname(sapply(
      seq_len(nrow(results_metrics_subdf)),
      function(i) {
        process_method_parameters_label(results_metrics_subdf[i, ],
          methods_labels,
          method_name = FALSE
        )
      }
    ...
    parameters_labels <- setNames(
      object = parameters_labels, nm =
        results_metrics_subdf$parameters
    )
    ...
    for (i in seq_len(nrow(parameters_combinations))) {
      ...
      if (method == "commensurate_power_prior") {
        # Compare each row of prior_parameters_df with the single row of
        # prior_param
        filter_df <- apply(
          prior_parameters_df, 1,
          function(row) compare_ignore_na(row, as.vector(prior_param))
        )
      } else {
      ...
      if (method == "commensurate_power_prior") {
        keys <- c(
          ...
          "heterogeneity_parameter_std", "power_parameter_mean",
          "power_parameter_std"
        )
        y <- unlist(posterior_parameters_df[filter_df, keys])
        conf_int_lower <- unlist(posterior_parameters_df[
          filter_df,
          paste0("conf_int_lower_", keys)
        ])
        conf_int_upper <- unlist(posterior_parameters_df[
          filter_df,
          paste0("conf_int_upper_", keys)
        ])
      } else {
        y <- unlist(posterior_parameters_df[filter_df, key])
        conf_int_lower <- unlist(posterior_parameters_df[
          filter_df,
          paste0("conf_int_lower_", key)
        ])
        conf_int_upper <- unlist(posterior_parameters_df[
          filter_df,
          paste0("conf_int_upper_", key)
        ])
      }
    ...
    x_range <- range(drift)
    x_width <- diff(x_range)
    # Set a relative cap size (necessary, otherwise the cap size will depend on
    # the x-axis width)
    cap_size <- x_width *
      relative_error_cap_width # Adjust relative_error_cap_width to control the relative cap size
    ...
    plt <- plt + geom_errorbar(
      ...
      scale_color_viridis_d(labels = parameters_labels) +
      # Apply the viridis color map for discrete data
    ...
    source_treatment_effect_estimate <-
      unique(results_metrics_df$source_treatment_effect_estimate)[1]
    mandatory_drift_values <-
      sort(important_drift_values(source_treatment_effect_estimate,
                                  case_study_config))

    mandatory_drift_values <- data.frame(
      drift_value = mandatory_drift_values,
      effect_label = c(
        "No effect", "Partially consistent effect",
        "Consistent effect"
      )
    )
...
posterior_parameters_plots <- function(results_metrics_df) {
  ...
  for (case_study in case_studies) {
    ...
    for (method in methods) {
      results_metrics_df2 <- results_metrics_df1[results_metrics_df1$method ==
                                                   method, ]
      target_sample_sizes <-
        unique(results_metrics_df2$target_sample_size_per_arm)
      for (target_sample_size_per_arm in target_sample_sizes) {
        results_metrics_df3 <-
          results_metrics_df2[
                              results_metrics_df2$target_sample_size_per_arm ==
                              target_sample_size_per_arm, ]

        target_to_source_std_ratios <-
          unique(results_metrics_df3$target_to_source_std_ratio)
        ...
        for (target_to_source_std_ratio in target_to_source_std_ratios) {
          ...
          source_denominator_change_factors <-
            unique(results_metrics_df4$source_denominator_change_factor)
