...
plot_success_proba_vs_drift <- function(metric,
  ...
  cap_size <- x_width * relative_error_cap_width /
  ...
  plt <- plt +
    geom_vline(
      xintercept = drift_theta_0,
      linetype = "dashed",
      color = "black"
    ) #+
  # annotate("text", x = drift_theta_0, y = 0, label = TeX("$\\theta_0 -
  # \\hat{\\theta}_0$"), vjust = -0.1 )
