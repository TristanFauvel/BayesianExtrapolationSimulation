analyze_power_gains <- function(results_freq_df, output_path) {
  # Check whether there are cases where the success proba is higher in the
  # alternative space than the frequentist power at equivalent TIE
  subdf <- subset(
    results_freq_df,
  ...
  # power of a separate analysis at equivalent TIE
  # power_gain_cases <- subset(subdf, conf_int_success_proba_lower >
  # frequentist_power_at_equivalent_tie_upper)

  subdf$se_success_proba <-
    (
     subdf$conf_int_success_proba_upper -
       subdf$conf_int_success_proba_lower) / (2 * 1.96)
  subdf$se_power_at_equivalent_tie <-
    (
     subdf$frequentist_power_at_equivalent_tie_upper -
       subdf$frequentist_power_at_equivalent_tie_lower) / (2 * 1.96)

  n <- 12
  # Apply the hypothesis test for each row in the dataframe
  subdf$p_value <- mapply(
    function(x, y, se_x, se_y) {
      # Perform a two-sample z-test using BSDA's z.test function
      # Determine whether the mean of the first sample (mean.x) is
      # significantly greater than the mean of the second sample (mean.y).
  ...
  # Determine whether the power of the method of interest is higher than the
  # power of the frequentist method at the nominal TIE (otherwise there is no
  # point in borrowing)
  power_gain_cases <- subset(
    power_gain_cases,
    conf_int_success_proba_lower > nominal_frequentist_power_separate
  )
  ...
  write.csv(
    file = paste0(output_path, "/", "power_gain_cases.csv"),
    power_gain_cases
  ) # , power_gain_cases[,expected_colnames_scenario])

  write.csv(file = paste0(
    output_path, "/",
    "power_gain_cases_separate_analysis.csv"
  ), separate_analysis_gain)
}

analyze_power_loss <- function(results_freq_df, output_path) {
  # Check whether there are cases where the success proba is lower in the
  # alternative space than the frequentist power at equivalent TIE while TIE is
  # inflated compared to the nominal TIE.
  subdf <- subset(
    results_freq_df,
    (null_space == "left" & target_treatment_effect > theta_0) |
      (null_space == "right" & target_treatment_effect < theta_0)
  ...
  power_loss_cases <- subset(
    subdf,
    conf_int_success_proba_upper < frequentist_power_at_equivalent_tie_lower
  )


  write.csv(
    file = paste0(output_path, "/", "power_loss_cases.csv"),
    power_loss_cases[, expected_colnames_scenario]
  )
}
...
analyze_power_loss_inflated_tie <- function(results_freq_df, output_path) {
  ...
  # power_loss_cases <- subset(subdf, conf_int_success_proba_upper <
  ...
  subdf$p_value <- mapply(
    function(x, y, se_x, se_y) {
      ...
      # determine whether the mean of the first sample (mean.x) is
      # significantly less than the mean of the second sample (mean.y).
      z_test <- BSDA::zsum.test(
  ...
  write.csv(file = paste0(
    output_path, "/",
    "power_loss_inflated_tie_cases.csv"
  ), power_loss_cases[
    ,
...
analyze_noninflated_tie <- function(results_freq_df, output_path) {
  ...
  non_inflated_tie_cases <- subset(non_inflated_tie_cases, drift ==
                                     -source_treatment_effect_estimate)

  # Remove the cases where there was no borrowing
  # filter <- non_inflated_tie_cases$conf_int_ess_moment_upper >= 0 &
  # non_inflated_tie_cases$conf_int_ess_moment_lower >= 0
  # non_inflated_tie_cases <- non_inflated_tie_cases[!filter,]


  write.csv(
    file = paste0(output_path, "/", "noninflated_tie_cases.csv"),
    non_inflated_tie_cases
  )
}
