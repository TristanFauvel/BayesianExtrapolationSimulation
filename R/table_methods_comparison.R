...
generate_comparison_table <- function(results_df,
  ...
  if (length(unique(df$target_treatment_effect)) !=
        length(important_target_treatment_effects)) {
    stop(
      paste0("The filtered dataframe cannot contain",
        " more unique target treatment effect", " values than there are in",
          " important_target_treatment_effects")
    )
  }
  ...
  combined_data <- dplyr::bind_rows(
    ...
    partially_consistent_effect_data |> dplyr::mutate(
      effect =
  ...
  wide_data_table <- data_table |>
    ...
    unite("No treatment effect", c(paste(
      x_metric_label,
      "No treatment effect"
    ), paste(
      metric$uncertainty_label,
      "No treatment effect"
    )), sep = " ", remove = TRUE) |>
    unite("Partially consistent", c(paste(
      x_metric_label,
      "Partially consistent"
    ), paste(
      metric$uncertainty_label,
    ...
    unite("Consistent", c(
      paste(x_metric_label, "Consistent"),
      paste(metric$uncertainty_label, "Consistent")
    ), sep = " ", remove = TRUE)

  export_table(
    data_table = wide_data_table, ncollapses = 1, column_names =
      c("Method", "Consistent", "No treatment effect", "Partially consistent"),
    title = paste(metric$label, title, sep = ", "), file_path = file_path
  )
}
...
#'
#' This function creates tables comparing methods with respect to different
#' metrics for various case studies and sample sizes.
#'
    ...
    for (case_study in case_studies) {
      ...
      for (target_sample_size_per_arm in target_sample_sizes) {
        ...
        source_denominator_change_factors <-
          unique(results_df2$source_denominator_change_factor)
        for (
          ...
          target_to_source_std_ratios <-
            unique(results_df3$target_to_source_std_ratio)
          for (target_to_source_std_ratio in target_to_source_std_ratios) {
            ...
            generate_comparison_table(results_df4, metric,
              target_to_source_std_ratio = target_to_source_std_ratio,
              source_denominator_change_factor =
                source_denominator_change_factor
            )
