...
binomial_cpp <- R6::R6Class(
  ...
  public = list(
    ...
    stan_prior_code = "
        data {
          int<lower = 0> n_treatment_source; //
            number of patients in the treatment group of the source study
          int<lower = 0> n_control_source; //
            number of patients in the control group of the source study

          int<lower = 0, upper =
            n_treatment_source> successes_treatment_source;  //
              number of successes in the treatment group of the source study
          int<lower = 0, upper = n_control_source> successes_control_source; /
            / number of successes in the control group of the source study
        ...
        parameters {
          ...
          real<lower = -fmin(control_rate_target, control_rate_source),
            upper = 1 - fmax(control_rate_target,
              control_rate_source)> target_treatment_effect; //
                The power prior approach assumes that the treatment effect is the same in the source and target studies
        }
        transformed parameters {
          real treatment_rate_source = control_rate_source +
            target_treatment_effect;
          real treatment_rate_target = control_rate_target +
            target_treatment_effect;
        }
        model {
          ...
          target_treatment_effect ~ uniform(-fmin(control_rate_target,
            control_rate_source), 1 - fmax(control_rate_target,
              control_rate_source)); // Initial prior on the treatment effect


          target += power_parameter *
            (binomial_lpmf(successes_control_source |n_control_source,
              control_rate_source) +
          binomial_lpmf(successes_treatment_source |n_treatment_source,
            treatment_rate_source)); // Likelihood of the source study p(D_S|
              target_treatment_effect)
    ...
    initialize = function(prior, mcmc_config) {
      ...
      self$stan_model_code <- "
        data {
          ...
          int<lower = 0> n_treatment_target; //
            number of patients in the treatment group of the target study
          int<lower = 0> n_control_target; //
            number of patients in the control group of the target study

          int<lower = 0, upper =
            n_treatment_target> successes_treatment_target;  //
              number of successes in the treatment group of the target study
          int<lower = 0, upper = n_control_target> successes_control_target; /
            / number of successes in the control group of the target study
        ...
        model {
          ...
          successes_control_target ~ binomial(n_control_target,
            control_rate_target);
          successes_treatment_target ~ binomial(n_treatment_target,
    ...
    prepare_data = function(target_data) {
      ...
      data_list <- list(
        ...
        n_treatment_source =
          as.integer(self$prior$source$sample_size_treatment),
...
#' Gaussian_NPP class
#'
#' @description This class represents a Gaussian model using the NPP
#'   (Noninformative Power Prior) approach.
#' It inherits from the Model class.
#'
#' @field power_parameter_mean The mean of the prior on the power parameter.
#' @field power_parameter_std The standard deviation of the prior on the power
#'   parameter.
#' @field target_treatment_effect_estimate Estimate of the treatment effect in
#'   the target study
#' @field target_treatment_effect_standard_error Standard error on the estimate
#'   of the treatment effect in the
#'   target study
...
#' @field max_posterior_pdf Maximum value of the posterior p.d.f., used for
#'   rejection sampling of the posterior p.d.f.
...
gaussian_npp <- R6::R6Class(
  ...
  public = list(
    ...
    initialize = function(prior) {
      ...
      self$power_parameter_mean <-
        prior$method_parameters$power_parameter_mean[[1]]
      self$power_parameter_std <-
        prior$method_parameters$power_parameter_std[[1]]

      if (self$power_parameter_mean < 0 ||
            self$power_parameter_mean > 1) {
        stop(
             "Invalid parameter for the mean of the prior on the power parameter")
      }

      if (self$power_parameter_std < 0) {
        stop(
             "Invalid parameter for the std of the prior on the power parameter")
      }


      if (self$power_parameter_std > 0.5 || self$power_parameter_std <= 0) {
        stop(
             "The standard deviation of a Beta distribution is between 0 and 0.5.")
      }
      ...
      omega <- self$power_parameter_std^2 / (xi *
        (
         1 - xi) - self$power_parameter_std^2)
      self$p <- xi / omega
      self$q <- (1 - xi) / omega

      # Because the behavior of the beta distribution changes a lot at 0 and 1
      # for p and q above or below 1, we do the following to avoid issues due
    ...
    unnormalized_posterior_power_parameter_pdf = function(power_parameter,
                                                          target_data) {
      ...
      if (any(is.na(pdf))) {
        stop(
             paste0("Unnormalized posterior PDF of the power",
               " parameter is evaluated to NA."))
      }
      pdf
    },
    ...
    power_parameter_posterior_pdf = function(power_parameter, target_data) {
      ...
      pdf <- self$unnormalized_posterior_power_parameter_pdf(
        power_parameter,
        target_data
      )
      ...
      if (any(is.na(norm_pdf))) {
        stop(
             "Normalized posterior PDF of the power parameter is evaluated to NA.")
      }
    ...
    normalizing_constant_power_parameter = function(target_data) {
      # Numerator of p(\power_parameter|D_T, D_S)
      int_fun <- function(power_parameter) {
        self$unnormalized_posterior_power_parameter_pdf(
          power_parameter,
          target_data
        )
      }
      ...
      if (normconst == 0) {
        stop(
             "Normalization constant of the power parameter posterior PDF is zero")
      }
      normconst
    },
    ...
    inference = function(target_data) {
      ...
      self$prior_normconst <-
        self$normalizing_constant_power_parameter(target_data)

      self$target_treatment_effect_estimate <-
        target_data$sample$treatment_effect_estimate
      self$target_treatment_effect_standard_error <-
        target_data$sample$treatment_effect_standard_error

      self$posterior_parameters$power_parameter_mean <- integrate(
        function(power_parameter) {
          power_parameter *
            self$unnormalized_posterior_power_parameter_pdf(power_parameter,
        ...
        upper = 1,
        rel.tol = 1e-8
      )$value / self$prior_normconst

      power_parameter_var <- integrate(
        function(power_parameter) {
          power_parameter^2 *
            self$unnormalized_posterior_power_parameter_pdf(power_parameter,
              target_data = target_data
        ...
        self$prior_normconst - self$posterior_parameters$power_parameter_mean^
        2
      ...
      bound <-
        Inf # 30 Using -Inf/Inf may result in error because of non-finite function value
      ...
      self$post_var <- integrate(
        ...
        lower = -bound,
        # Using -Inf/Inf would result in error because of non-finite function value
        upper = bound,
        rel.tol = 1e-8,
        stop.on.error = FALSE
      ...
      if (self$post_var == 0) {
        stop(
             paste0("Posterior variance is 0. Most likely due",
               " to numerical errors when computing the",
                 " integral. One solution is to adapt the",
                   " integration range."))
      }
    ...
    sample_posterior = function(n_samples) {
      ...
      while (count < n_samples) {
        x_proposal <- rnorm(
          1, self$post_mean,
          sqrt(self$post_var)
        ) # Sampling from the proposal distribution q(x)
    ...
    sample_prior = function(n_samples) {
      ...
      power_parameter_samples <- rbeta(n_samples,
        shape1 = self$p, shape2 =
          self$q
      )

      samples <- numeric(n_samples)

      # Sample theta from the normal distribution for each sampled
      # power_parameter
      for (i in 1:n_samples) {
        sigma <- sqrt(self$prior$source$standard_error^2 /
                        power_parameter_samples[i])
        samples[i] <- rnorm(1,
          mean = self$prior$source$treatment_effect_estimate,
          sd = sigma
        )
    ...
    #' @description Calculate the posterior cumulative distribution function
    #'   (CDF).
    ...
    posterior_pdf = function(x) {
      # marginal posterior of theta p(\theta| D_T, D_S): integration of
      # p(\theta, power_parameter | D_T, D_S) with respect to power_parameter
      if (is.null(self$prior_normconst)) {
        stop(
             "Call self$inference(target_data) before accessing posterior_pdf().")
      }
    ...
    prior_pdf = function(x) {
      # p(\theta|D_S): integration of p(\theta, power_parameter | D_S) with
      # respect to power_parameter
    ...
    #' @description Plot posterior probability density function (PDF) of the
    #'   power parameter
    #' @param target_data Target study data
    #' @return A plot
    plot_power_parameter_posterior_pdf = function(target_data) {
      ...
      posterior_pdf <- model$power_parameter_posterior_pdf(
        x_values,
        target_data
      )
    ...
    plot_power_parameter_vs_drift = function(source_treatment_effect_estimate,
      ...
      ggplot(plot_data, aes(x = Drift, y = PowerParameter)) +
        ...
        labs(
          title = "Power Parameter as a Function of Drift", x = "Drift",
          y = "Power Parameter"
        ) +
        theme_minimal()
    }
...
#'
#' @field method Method name
#' @field heterogeneity_prior_family Heterogeneity prior family (half_normal,
#'   inverse_gamma)
#'
#' @export
gaussian_commensurate_power_prior <- R6::R6Class(
  ...
  public = list(
      ...
      self$stan_model_code <- "
                                      functions {
                                        real g_function(real log_tau) {
                                          return fmax(log_tau, 1); //
                                            Define g(τ)
                                        }
                                      }

                                      data {
                                        int<lower=0,upper=2> prior_type; //
                                          Type of prior on the commensurability parameter; 0 for inverse_gamma, 1 for half normal, 2 for Cauchy
                                        int<lower=0> NS;       //
                                          Number of samples in dataset S
                                        int<lower=0> NT;       //
                                          Number of samples in dataset T
                                        real<lower=
                                          0> target_sampling_variance;  //
                                            Known variance of the target data
                                        real<lower=0> prior_variance; //
                                        ...
                                        real std_dev; //
                                          Parameter for the half normal prior
                                        real location; //
                                          Location parameter for the Cauchy prior
                                        real scale; //
                                          Scale parameter for the Cauchy prior
                                        real alpha; //
                                          Parameter for the inverse_gamma prior
                                        real beta; //
                                          Pocation parameter for the inverse_gamma prior
                                      }
                                      ...
                                      transformed parameters {
                                        real<lower=0>  u = power_parameter *
                                          NS + tau * prior_variance;
                                      ...
                                      model {
                                        ...
                                        power_parameter ~ beta(g_function(
                                          log_tau), 1);

                                        target_treatment_effect ~ normal(
    (power_parameter * NS * tau * target_sampling_variance *
      source_treatment_effect_estimate + NT * u *
        target_treatment_effect_estimate) /
    (power_parameter * NS * tau * target_sampling_variance + NT * u),
    sqrt((u * target_sampling_variance) / (power_parameter * NS * tau *
      target_sampling_variance + NT * u)));
                                      }
                                      "

      self$heterogeneity_prior_family <-
        prior$method_parameters$heterogeneity_prior$family

      model_name <- paste0(
        "gaussian_commensurate_pp_",
        self$heterogeneity_prior_family
      )
      self$stan_model <- compile_stan_model(model_name, self$stan_model_code)
    ...
    prepare_data = function(target_data) {
      ...
      data <- list(
        ns =
          as.integer(self$prior$source$equivalent_source_sample_size_per_arm),
        # Number of samples in dataset S
        ...
        target_treatment_effect_estimate =
          target_data$sample$treatment_effect_estimate # Estimate from target dataset
      )

      heterogeneity_prior <- self$prior$method_parameters$heterogeneity_prior

      # Initialize parameters list based on the prior family
      parameters <- switch(self$heterogeneity_prior_family,
        half_normal = list(
          prior_type = 1,
    ...
    compute_posterior_parameters = function() {
      self$posterior_parameters <- list(
        heterogeneity_parameter_mean =
          self$fit_summary[self$fit_summary$variable == "tau", "mean"] |>
          dplyr::pull(mean),
        heterogeneity_parameter_std =
          self$fit_summary[self$fit_summary$variable == "tau", "sd"] |>
          dplyr::pull(sd),
        power_parameter_mean = self$fit_summary[self$fit_summary$variable ==
                                                "power_parameter", "mean"] |>
        dplyr::pull(mean),
        power_parameter_std = self$fit_summary[self$fit_summary$variable ==
                                               "power_parameter", "sd"] |>
        dplyr::pull(sd)
      )
    },
    #' @description Draw samples from the prior distribution. Based on equation
    #'   8 in Hobbs et al (2011).
    #' @param n_samples Number of samples
    sample_prior = function(n_samples) {
      heterogeneity_prior <- self$prior$method_parameters$heterogeneity_prior
      if (self$heterogeneity_prior_family == "cauchy") {
        log_tau_samples <- rcauchy(
          n = n_samples, location =
            heterogeneity_prior$location, scale = heterogeneity_prior$scale
        )
        tau_samples <- exp(log_tau_samples)
      } else if (self$heterogeneity_prior_family == "half_normal") {
        tau_samples <- extraDistr::rhnorm(
          n = n_samples, sigma =
            heterogeneity_prior$std_dev
        )
        ...
        tau2_samples <- extraDistr::rinvgamma(
          n = n_samples, alpha =
            heterogeneity_prior$alpha, beta = heterogeneity_prior$beta
        )
      ...
      power_parameter_samples <- rbeta(
        n_samples, g_function(log_tau_samples),
        1
      ...
      treatment_effect_samples
    },
    #' @description Joint prior p.d.f. Based on equation (8) in Hobbs et al
    #'   (2011).
    ...
    joint_prior_pdf = function(treatment_effect, gamma, tau) {
      ...
      if (self$heterogeneity_prior_family == "cauchy") {
        prior_tau <- dcauchy(log_tau,
          location = heterogeneity_prior$location,
          scale = heterogeneity_prior$scale
        ) / tau # Adjust for the Jacobian of the transformation
      } else if (self$heterogeneity_prior_family == "inverse_gamma") {
        tau2 <- tau^2

        prior_tau <- 2 * tau * extraDistr::dinvgamma(tau2,
          alpha = heterogeneity_prior$alpha,
          beta = 1 / heterogeneity_prior$beta
        ) # Adjust for the Jacobian of the transformation
      } else if (self$heterogeneity_prior_family == "half_normal") {
        prior_tau <- extraDistr::dhnorm(tau,
          sigma =
            heterogeneity_prior$std_dev
        )
      } else {
      ...
      normal_component * beta_component * prior_tau
    },

    #' @description Integrate out gamma and tau to get the marginal PDF for
    #'   treatment_effect
    #' @param treatment_effect Treatment effect
    ...
    prior_pdf = function(treatment_effect) {
      ...
      normalized_pdf_treatment_effect <-
        unnormalized_prior_pdf_vec(treatment_effect) / normalizing_constant
      normalized_pdf_treatment_effect
    }
  )
)
