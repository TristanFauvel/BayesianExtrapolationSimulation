...
#'
#' @description This function calculates the standard error of the log odds
#'   ratio based on the number of noncases and cases in the
...
#' @param n_control_responders Number of cases in the unexposed group
#' @param n_treatment_responders Number of cases in the exposed group
#' @param continuity_correction Logical; if TRUE, applies continuity correction
#'   (default is TRUE)
#'
#' @return The standard error of the log odds ratio
#'
...
standard_error_log_odds_ratio <- function(n_control_nonresponders,
  ...
  sqrt(
    1 / n_control_nonresponders + 1 / n_treatment_nonresponders + 1 /
      n_control_responders + 1 / n_treatment_responders
  )
}
...
#'
#' @description This function generates binary data (0s and 1s) based on a
#'   given rate and sample size.
#'
...
generate_binary_data_from_rate <- function(rate, sample_size) {
  ...
  c(rep(0, times = n_failures), rep(1, times = n_successes))
}

#' Calculate the success rate in the target study arm from the drift on the log
#' odds ratio scale
#'
#' @description This function calculates the success rate in the target study
#'   arm based on the drift (defined on the log scale) and the
#'   success rate in the arm of interest in the source study.
#'
#' @param arm_drift The drift (defined on the log scale) between the target and
#'   source study arms
#' @param source_rate The success rate in the arm of interest in the source
#'   study
#'
#' @return The success rate in the corresponding arm of the target study
#'
#' @keywords internal
rate_from_drift_log_or <- function(arm_drift, source_rate) {
...
#' Calculate the rate in the target study arm from the drift on the log
#' relative risk scale
#'
#' @description This function calculates the rate in the target study arm based
#'   on the drift (defined on the log scale) and the rate in
#'   the arm of interest in the source study.
#'
#' @param arm_drift The drift (defined on the log scale) between the target and
#'   source study arms
...
#' @keywords internal
rate_from_drift_log_rr <- function(arm_drift, source_rate) {
  source_rate * exp(arm_drift)
}
...
#' Compute odds ratios based on the number of participants and responders in
#' each arm
#'
#' @description This function computes odds ratios based on the number of
#'   participants and responders in the control and treatment
#'   arms.
#'
...
#'
#' @return A list containing the log odds ratio, treatment rate, control rate,
#'   and standard error of the log odds ratio
#'
#' @export
#' @keywords internal
compute_o_rs <- function(n_control,
                        n_treatment,
                        n_control_responders,
                        n_treatment_responders) {
...
compute_log_odds_ratio_from_counts <- function(n_control_responders,
  ...
  odds_ratio <- (n_treatment_responders * n_control_nonresponders) /
    (n_treatment_nonresponders * n_control_responders)
  log_odds_ratio <- log(odds_ratio)
  log_odds_ratio
}

#' Sample log odds ratios based on the number of participants and responders in
#' each arm
#'
#' @description This function samples log odds ratios based on the number of
#'   participants and responders in the control and treatment
#'   arms.
#'
#' @param n_control Number of participants in the control arm
...
#'
#' @return A list containing the sampled log odds ratios and the standard error
...
#'
#' @description This function samples rate ratios based on the rates in the
...
sample_rate_ratios <- function(control_rate,
                               ...
                               n_control,
                               n_treatment) {
  sample_treatment_rate <- sample_aggregate_binary_data(
    treatment_rate,
    n_treatment, n_replicates
  )
  sample_control_rate <- sample_aggregate_binary_data(
    control_rate, n_control,
    n_replicates
  )

  sample_treatment_rate / sample_control_rate
}

#' Sample aggregate normal data based on the mean and variance
#'
#' @description This function samples aggregate normal data based on the mean
#'   and variance.
#'
...
#'
#' @return A list containing the sample mean and sample standard error for each
#'   replicate
#' @export
sample_aggregate_normal_data <- function(mean,
  ...
  if (n_samples_per_arm > 1) {
    ...
    sample_variance <- variance * rchisq(n_replicates,
      df =
        degrees_of_freedom
    ) / degrees_of_freedom
  } else {
  ...
  sample_standard_error <- sqrt(sample_variance /
                                  n_samples_per_arm) # standard error on the mean, for each replicate
...
#'
#' @description This function samples aggregate binary data based on the rate
#'   and sample size.
#'
#' @param rate The rate of success (1s) in the generated data
...
#' SourceData class
#'
#' @description This class represents the source data used for Bayesian
#'   borrowing. It processes the source data to compute
#'   necessary statistics for various endpoints.
#'
#' @importFrom R6 R6Class
#' @importFrom stats pexp
#'
#' @export
source_data <- R6::R6Class("SourceData",
  inherit = observed_source_data,
  public = list(
    #' @description Initialize the SourceData object
    #'
    #' @param case_study_config list: A configuration list containing details
    #'   of the case study.
    #' @param source_denominator numeric: An optional denominator for computing
    #'   control rates.
    #'
    ...
    initialize = function(case_study_config, source_denominator = NA) {
      ...
      if (self$endpoint == "binary") {
        if (self$summary_measure_likelihood == "normal") {
          # Observed source data (required for the treatment effect estimate in
          # the source data):
          ...
          n_responders_treatment <- self$treatment_rate *
            self$sample_size_treatment
          self$standard_error <- sqrt(
            1 / n_responders_treatment + 1 /
              (self$sample_size_treatment - n_responders_treatment) + 1 /
              n_responders_control + 1 /
              (self$sample_size_control - n_responders_control)
          )
        ...
        self$treatment_rate <- self$control_rate *
          exp(self$treatment_effect_estimate)

        # The expected number of events in each arm is P(time_to_first_event <=
        # max_time) * N_participants
        ...
        n_treatment <-
          case_study_config$source$treatment # Sample size for the first rate
        n_control <-
          case_study_config$source$control # Sample size for the second rate

        # Compute the standard deviations for the negative binomial
        # distributions
        sigma_treatment <- sqrt(negative_binomial_variance(
          mu_treatment,
          k_treatment
        ))
        ...
        self$standard_error <- sqrt((se_r_treatment / mu_treatment)^2 +
                                      (se_r_control / mu_control)^
                                        2)
      }
    }
  )
...
#'
#' @description This class represents the observed source data used for
#'   Bayesian borrowing in clinical studies.
#'
#' @field endpoint character. The endpoint of the study. Possible values are
#'   "continuous", "binary", "time_to_event", "recurrent_event".
#' @field summary_measure_likelihood character. The distribution of the summary
#'   measure. Possible values are "normal",
#'   "Binomial".
#' @field sample_size_control numeric. The sample size in the control arm of
#'   the source data.
#' @field sample_size_treatment numeric. The sample size in the treatment arm
#'   of the source data.
#' @field treatment_effect_estimate numeric. The estimated treatment effect in
#'   the source data.
#' @field standard_error numeric. The standard error of the treatment effect
#'   estimate in the source data.
#' @field control_rate numeric. The rate of success (1s) in the control arm of
#'   the source data (for binary endpoint).
#' @field treatment_rate numeric. The rate of success (1s) in the treatment arm
#'   of the source data (for binary endpoint).
#' @field equivalent_source_sample_size_per_arm numeric. Equivalent sample size
#'   per arm.
#'
#' @export
observed_source_data <- R6::R6Class(
  "ObservedSourceData",
  public = list(
    endpoint = NULL,
    ...
    #'
    #' @param case_study_config list. A list containing the configuration for
    #'   the case study.
    ...
    initialize = function(case_study_config) {
      ...
      self$equivalent_source_sample_size_per_arm <- 2 *
        self$sample_size_control * self$sample_size_treatment /
        (self$sample_size_control + self$sample_size_treatment)


      if (self$endpoint == "continuous") {
        self$treatment_effect_estimate <-
        ...
        self$control_rate <- case_study_config$source$responses$control /
          self$sample_size_control
        self$treatment_rate <- case_study_config$source$responses$treatment /
        ...
        if (self$summary_measure_likelihood == "binomial") {
          self$treatment_effect_estimate <-
            self$treatment_rate - self$control_rate
          self$standard_error <- sqrt(
            self$treatment_rate * (1 - self$treatment_rate) /
              self$sample_size_treatment + self$control_rate *
              (1 - self$control_rate) / self$sample_size_control
      ...
      if (self$endpoint == "continuous") {
        source_dict <- list(
          ...
          equivalent_source_sample_size_per_arm =
            self$equivalent_source_sample_size_per_arm,
...
#' @description A factory class for creating different types of target data
#'   objects.
#' @export
target_data_factory <- R6::R6Class("TargetDataFactory", public = list(
  #' @description Creates a target data object based on the source data and
  #'   configuration.
  ...
  #'   study sampling standard deviation
  ...
  create = function(source_data,
    ...
    if (source_data$endpoint == "continuous") {
      ...
      target_data <- continuous_target_data$new(
      ...
      target_data <- binary_target_data$new(
        source_data = source_data,
        sampling_approximation = sampling_approximation,
        target_sample_size_per_arm = target_sample_size_per_arm,
        control_drift = control_drift,
      ...
      target_data <- recurrent_event_target_data$new(
        source_data = source_data,
        sampling_approximation = sampling_approximation,
        target_sample_size_per_arm = target_sample_size_per_arm,
        control_drift = control_drift,
      ...
      target_data <- time_to_event_target_data$new(
        source_data = source_data,
        sampling_approximation = sampling_approximation,
        target_sample_size_per_arm = target_sample_size_per_arm,
        control_drift = control_drift,
...
#' @field drift The drift value calculated as the difference between treatment
#'   and control drifts.
...
#' @field sampling_approximation The flag indicating if sampling approximation
#'   is used.
#' @field standard_deviation The target standard deviation value.
#' @field sample The sample data.
#' @export
target_data <- R6::R6Class(
  "TargetData",
  public = list(
    endpoint = NULL,
    ...
    initialize = function(source_data,
                          ...
                          treatment_drift,
                          summary_measure_likelihood) {
      assertions::assert_class(source_data, c(
        "SourceData",
        "ObservedSourceData"
      ))
      ...
      self$treatment_effect <- self$drift +
      ...
      p3 <- ggplot(
        ...
        ggplot2::labs(
          x = "Treatment Effect Estimate", y =
            "Treatment Effect Standard Error", fill = "Count"
        )

      # # Plot 3: Scatter plot of treatment effect estimate vs standard error
      # p3 <- ggplot(data, aes(x = treatment_effect_estimate, y =
...
#' @export
continuous_target_data <- R6::R6Class(
  ...
  public = list(
    ...
    initialize = function(source_data,
      ...
      self$standard_deviation <- target_to_source_std_ratio *
        source_data$standard_error *
        sqrt(
             source_data$equivalent_source_sample_size_per_arm) # This is the sampling standard deviation for pairs of individual patients data.
  ...
  public = list(
    ...
    initialize = function(source_data,
      ...
      if (summary_measure_likelihood == "binomial") {
        ...
        # We compute the rate in each arm, based on the drift (defined on the
        # log scale)
        self$control_rate <- rate_from_drift_log_or(
          control_drift,
          source_data$control_rate
        )
        self$treatment_rate <- rate_from_drift_log_or(
          treatment_drift,
          source_data$treatment_rate
        )
      } else {
        stop("Not implemented for other distributions")
      }
      ...
      stopifnot(all.equal(self$treatment_rate, min(max(
        self$treatment_rate,
        0
      ), 1)))
      ...
      if (summary_measure_likelihood == "binomial") {
        ...
        treatment_effect_standard_error <- sqrt((
          self$treatment_rate * (1 - self$treatment_rate) /
            ...
            (1 - self$control_rate) / self$sample_size_treatment
        ))
        self$standard_deviation <- treatment_effect_standard_error *
          sqrt(target_sample_size_per_arm)
      } else if (summary_measure_likelihood == "normal") {
      ...
      if (summary_measure_likelihood == "binomial") {
        ...
        self$treatment_effect <- self$drift +
          source_data$treatment_effect_estimate
        # We make sure that target treatment effect is consistent with the
        # rates in each arm, otherwise this means there is a mistake in the
        # implementation
        # if (abs(compute_log_odds_ratio(self$treatment_rate,
        # self$control_rate)) != Inf && abs(self$treatment_effect -
        # compute_log_odds_ratio(self$treatment_rate, self$control_rate)) >
        # 1e-3) {
        # stop("The treatment effect (log odds ratio) is not consistent with
        # the drift")
        # }
      }
      assertions::assert_number(self$treatment_effect)
    ...
    generate = function(n_replicates) {
      if (self$summary_measure_likelihood == "normal" &&
        ...
        treatment_effect_standard_error <- sqrt(
          sample_treatment_rate * (1 - sample_treatment_rate) /
            self$sample_size_control + sample_control_rate *
            (1 - sample_control_rate) / self$sample_size_treatment
        )

        standard_deviation <- sqrt(
          sample_treatment_rate * (1 - sample_treatment_rate) +
            sample_control_rate * (1 - sample_control_rate)
        )
        ...
        samples <- data.frame(
          ...
          treatment_effect_standard_error =
            log_or_samples$std_err_log_odds_ratio,
          sample_size_per_arm = self$sample_size_per_arm,
          standard_deviation = log_or_samples$std_err_log_odds_ratio *
          sqrt(self$sample_size_per_arm)
        )
      } else {
        stop("Not implemented for other distributions")
...
#
# The estimate is parameterised by 1 / theta rather than theta so that the
# Poisson limit is the finite boundary value 0 rather than an infinity. The
# profile likelihood is maximised there whenever the score test for
# overdispersion, sum((y - ybar)^2) <= n * ybar, does not reject the Poisson
# model; otherwise the maximum is interior and is located by optimising the
# profile log likelihood over log(theta).
negative_binomial_inverse_dispersion <- function(input_data, rate_estimate) {
  ...
  if (sum((input_data - rate_estimate)^2) <= n_observations * rate_estimate) {
    return(0)
  }

  # The profile log likelihood depends on the data only through the counts of
  # each observed value, which keeps every evaluation cheap.
  counts <- tabulate(input_data + 1L)
  values <- seq_along(counts) - 1
  observed <- counts > 0
  counts <- counts[observed]
  values <- values[observed]
  total_count <- sum(counts * values)

  negative_profile_loglikelihood <- function(log_theta) {
    theta <- exp(log_theta)
    -(sum(counts * (lgamma(values + theta) - lgamma(theta))) +
        n_observations * theta * log(theta / (theta + rate_estimate)) +
        total_count * log(rate_estimate / (rate_estimate + theta)))
  }

  optimum <- stats::optimize(
    negative_profile_loglikelihood,
    interval = c(log(1e-8), log(1e8)),
    tol = 1e-8
  )

  1 / exp(optimum$minimum)
}
...
#'
#' @param input_data The data for which the rate parameter and standard error
#'   need to be estimated.
#' @return A list containing the rate estimate, standard error of the rate
#'   estimate, and standard error of the log(rate) estimate.
#' @export
negative_binomial_regression <- function(input_data) {
  if (all(input_data == 0)) {
    warning(
            paste0("All values are zero. Negative binomial",
              " regression will fail. Adding random", " noise."))
    futile.logger::flog.warn(
                             paste0("All values are zero. Negative binomial",
                               " regression will fail. Adding random",
                                 " noise."))
    input_data[sample(seq_along(input_data), 1)] <- 1
  }
...
recurrent_event_target_data <- R6::R6Class(
  inherit = target_data,
  public = list(
    control_rate = NULL,
    treatment_rate = NULL,
    ...
    #' @param sampling_approximation A flag indicating whether to use sampling
    #'   approximation.
    #' @param target_sample_size_per_arm The target sample size per arm.
    #' @param control_drift The control drift.
    #' @param treatment_drift The treatment drift.
    #' @param summary_measure_likelihood The summary measure distribution.
    #' @param k_treatment Size parameter for the negative binomial distribution
    #'   in the treatment arm.
    #' @param k_control Size parameter for the negative binomial distribution
    #'   in the control arm.
    initialize = function(source_data,
                          sampling_approximation,
                          target_sample_size_per_arm,
      ...
      self$control_rate <- rate_from_drift_log_rr(
        control_drift,
        source_data$control_rate
      )
      self$treatment_rate <- rate_from_drift_log_rr(
        treatment_drift,
        source_data$treatment_rate
      )
      ...
      assertions::assert_number(self$control_rate)
      assertions::assert_number(self$treatment_rate)
      # We make sure that target treatment effect is consistent with the rates
      # in each arm, otherwise this means there is an implementation mistake
      if (abs(self$treatment_effect - log(self$treatment_rate /
                                            self$control_rate)) > 1e-3) {
        stop(
             paste0("The treatment effect (log rates ratio)",
               " is not consistent with the drift"))
      }
      ...
      if (self$summary_measure_likelihood == "normal") {
        ...
        sigma_control <- sqrt(negative_binomial_variance(
          mu_control,
          self$k_control
        ))
        ...
        self$standard_deviation <- sqrt(self$sample_size_per_arm) *
          sqrt((se_r_treatment / mu_treatment)^
                 2 +
                 (se_r_control / mu_control)^2)

        assertions::assert_number(self$standard_deviation)
      } else if (self$summary_measure_likelihood != "normal") {
        stop(
             paste0("Not implemented for treatment effect",
    ...
    #'
    #' @description This function generates target data based on the specified
    #'   parameters.
    #'
    #' @param n_replicates The number of replicates to generate.
    ...
    generate = function(n_replicates) {
      if (self$summary_measure_likelihood == "normal" &&
        ...
        for (i in 1:n_replicates) {
          # Generate the patient-level data by sampling from negative binomial
          # distributions
          ...
          treatment <- negative_binomial_regression(
            input_data =
              target_data_treatment
          )

          control <- negative_binomial_regression(
            input_data =
              target_data_control
          )

          log_rate_ratio <-
            log(treatment$rate_estimate) - log(control$rate_estimate)

          # Calculate the SE of the rate ratio using the delta method
          se_log_rate_ratio <- sqrt(treatment$se_log_rate^2 +
                                      control$se_log_rate^
                                        2)
        ...
        samples <- data.frame(
          ...
          sample_size_per_arm = self$sample_size_per_arm,
          standard_deviation = se_log_rate_ratios *
            sqrt(self$sample_size_per_arm)
        )
      } else {
        stop("Not implemented for other distributions")
...
sample_exponential_arm_statistics <- function(n_subjects,
  ...
  if (total_events > 0) {
    ...
    exposure_from_events[as.integer(rownames(per_replicate))] <-
      per_replicate[, 1]
  }
  ...
  list(n_events = n_events, exposure = exposure)
}
...
#'
#' @export
time_to_event_target_data <- R6::R6Class(
  inherit = target_data,
  public = list(
    control_rate = NULL,
    treatment_rate = NULL,
    ...
    initialize = function(source_data,
      ...
      # The drift/treatment drift/control drift is defined on the log scale, so
      # we convert them back to the natural scale:
    ...
    generate = function(n_replicates) {
      ...
      if (self$summary_measure_likelihood == "normal" &&
            self$sampling_approximation == TRUE) {
        # Sample number of events over the follow-up period in each arm
        # according to a Poisson distributions.
        ...
        sampling_standard_deviation <- se_log_rr *
          sqrt(self$sample_size_per_arm)
        ...
        se_log_rate_ratios <- sqrt(1 / control$n_events + 1 /
                                     treatment$n_events)
...
#' @description A wrapper used for analyzing observed data (not used for
#'   simulation)
...
#' @export
observed_target_data <- R6::R6Class(
  "ObservedTargetData",
  public = list(
    summary_measure_likelihood = NULL,
    sample_size_per_arm = NULL,
    ...
    #' @description Initializes an ObservedTargetData object with the given
    #'   parameters
    #' @param treatment_effect_estimate The estimated treatment effect
    #' @param treatment_effect_standard_error The standard error of the
    #'   treatment effect estimate
    #' @param target_sample_size_per_arm The target sample size per treatment
    #'   arm
    #' @param summary_measure_likelihood The distribution of the summary
    #'   measure (e.g., "normal", "binomial")
    ...
    initialize = function(treatment_effect_estimate,
      ...
      self$sample$treatment_effect_standard_error <-
        treatment_effect_standard_error
    }
  )
)
