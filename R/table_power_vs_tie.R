...
#'
#' This function creates a table comparing power (or power difference) and type
#' I error (TIE)
...
#'   the results.
#' @param treatment_effect Character string specifying the treatment effect:
#'   "consistent", "partially_consistent", or
#'   "no_effect".
#' @param power_difference Logical; if TRUE, calculate power difference instead
#'   of power.
#' @param metrics A vector of metric names (not used in the current
#'   implementation).
#'
#' @return This function doesn't return a value but generates and saves tables
...
#'   implementation).
#'
#' @return This function doesn't return a value but generates multiple tables
#'   using table_power_vs_tie().
#'
#' @import dplyr
#'
#' @export
power_vs_tie_tables <- function(results_metrics_df, metrics) {
  ...
  for (case_study in case_studies) {
    ...
    target_sample_sizes <-
      unique(results_metrics_df$target_sample_size_per_arm[
                                                           results_metrics_df$case_study == case_study])
    ...
    for (target_sample_size in target_sample_sizes) {
      for (treatment_effect in treatment_effects) {
        for (source_denominator_change_factor in unique(
                                                        results_metrics_df$source_denominator_change_factor)) {
          for (target_to_source_std_ratio in unique(
                                                    results_metrics_df$target_to_source_std_ratio)) {
            table_power_vs_tie(
              results_metrics_df,
              case_study,
              target_sample_size,
