...
forest_subplot_sweet_spot <- function(data,
  ...
  data <- data |>
    unnest(cols = c(
      !!rlang::sym(sweet_spot_lower),
      !!rlang::sym(sweet_spot_upper)
    ))
  ...
  if (sort_by == "values") {
    ...
    widths <- data_grouped |>
      summarize(
        total_width =
          sum(!!rlang::sym(sweet_spot_upper) - !!rlang::sym(sweet_spot_lower),
            na.rm = TRUE
          ),
        .groups = "drop"
      ) |>
  ...
  source_treatment_effect_estimate <-
    unique(data$source_treatment_effect_estimate)[1]
...
forest_plot_sweet_spot <- function(results_freq_df, x_metric) {
  ...
  id_cols <- c(
    ...
    "case_study", "sampling_approximation",
    "source_treatment_effect_estimate", "target_to_source_std_ratio",
    "theta_0", "null_space", "summary_measure_likelihood",
    "source_standard_error", "source_sample_size_control",
    "source_sample_size_treatment",
    "equivalent_source_sample_size_per_arm", "endpoint",
    "source_control_rate", "source_treatment_rate"
  )

  results_freq_df <- results_freq_df |>
    pivot_wider(
      id_cols = id_cols, names_from = metric, values_from =
        c(
          "sweet_spot_lower", "sweet_spot_upper", "sweet_spot_width",
          "drift_range_upper", "drift_range_lower"
        )
    )
  ...
  title <- format_title(
    title = title, case_study = unique(df$case_study),
    target_to_source_std_ratio = unique(df$target_to_source_std_ratio),
    source_denominator_change_factor =
      unique(df$source_denominator_change_factor)
  )

  x_range <- c(
    unique(df[, paste0("drift_range_lower_", metric$name)]),
    unique(df[, paste0("drift_range_upper_", metric$name)])
  )
...
forest_plot_sweet_spots_comparison <- function(sweet_spots_df, metrics) {
  ...
  for (metric in metrics) {
    for (case_study in case_studies) {
      sweet_spots_df1 <- sweet_spots_df[sweet_spots_df$case_study ==
                                          case_study, ]
      ...
      for (target_sample_size_per_arm in target_sample_sizes) {
        sweet_spots_df2 <-
          sweet_spots_df1[sweet_spots_df1$target_sample_size_per_arm ==
                          target_sample_size_per_arm, ]

        target_to_source_std_ratios <-
          unique(sweet_spots_df2$target_to_source_std_ratio)
        ...
        for (target_to_source_std_ratio in target_to_source_std_ratios) {
          ...
          source_denominator_change_factors <-
            unique(sweet_spots_df3$source_denominator_change_factor)
...
plot_sweet_spot_width_metric_vs_sample_size <- function(metric,
  ...
  sweet_spot_df <- sweet_spot_df[
    ,
    !colnames(sweet_spot_df) %in% c(
      "sweet_spot_lower", "sweet_spot_upper",
      "sweet_spot_width"
    )
  ]
  ...
  tryCatch(
    {
      # Convert to numeric
      results_df[["sweet_spot_total_width"]] <-
        as.numeric(unlist(results_df[["sweet_spot_total_width"]]))
      results_df[["sweet_spot_total_width"]] <-
        as.numeric(unlist(results_df[["sweet_spot_total_width"]]))
    },
    error = function(e) {
  ...
  for (method in unique(results_df$method)) {
    parameters <- get_parameters(results_df[results_df$method == method,
                                   "parameters",
                                   drop = FALSE
                                 ])
    ...
    for (key in names(methods_dict[[method]])) {
      ...
      if (method == "commensurate_power_prior") {
        filter <- (parameters$heterogeneity_prior.family == "half_normal" &
                     parameters$heterogeneity_prior.std_dev == 1) |
          (parameters$heterogeneity_prior.family == "inverse_gamma" &
             parameters$heterogeneity_prior.alpha == 0.3333)
      } else {
  ...
  plt <- ggplot(mapping = aes_string(
    x = "target_sample_size_per_arm", y =
      "sweet_spot_total_width"
  ))
...
#'
#' @examples NA
plot_sweet_spot_width_vs_scenario <- function(
  sweet_spot_df,
  sweet_spots_metrics
) {
  ...
  for (case_study in case_studies) {
    ...
    for (metric in sweet_spots_metrics) {
      for (source_denominator_change_factor in unique(
                                                      sweet_spot_df1$source_denominator_change_factor)) {
        sweet_spot_df2 <- sweet_spot_df1 |>
          dplyr::filter(source_denominator_change_factor ==
                          !!source_denominator_change_factor |
                          is.na(source_denominator_change_factor))

        for (target_to_source_std_ratio in unique(
                                                  sweet_spot_df2$target_to_source_std_ratio)) {
