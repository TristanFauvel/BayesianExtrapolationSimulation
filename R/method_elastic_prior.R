...
#'
#' @description This class represents a Gaussian Elastic Prior model for
#'   logistic regression.
...
#' @field target_treatment_effect_estimate Numeric. Treatment effect estimate
#'   of the target study.
#' @field target_sample_size_per_arm Numeric. Sample size per arm in the target
#'   study.
#' @field source_sample_size_per_arm Numeric. Sample size per arm in the source
#'   study.
#' @field source_data_variance Numeric. Variance of the source data.
#' @field method Character. Method used for the analysis (default:
#'   "elastic_prior").
#' @field shape_param Numeric. Shape parameter for the elastic function.
#' @field prior List. Prior information for the analysis.
#'
#' @export
gaussian_elastic_prior_logistic <- R6::R6Class(
  ...
  public = list(
    ...
    initialize = function(prior, summary_measure_likelihood) {
      ...
      self$source_sample_size_per_arm <-
        prior$source$equivalent_source_sample_size_per_arm

      self$source_data_variance <- prior$source$standard_error^2 *
        self$source_sample_size_per_arm

      self$shape_param <- prior$method_parameters$shape_param[[1]]
    },

    #' @description Updates the vague prior variance based on empirical Bayes
    #'   approach.
    #'
    ...
    method_calibration = function(target_data) {
      ...
      tuning_params <- self$determine_tuning_parameters(
        target_data =
          target_data
      )
    ...
    #' @description Determines tuning parameters a and b for the logistic
    #'   elastic function.
    #'
    #' @param target_data List. The target data for the analysis.
    #'
    ...
    determine_tuning_parameters = function(target_data) {
      ...
      clinically_meaningful_diff <-
        self$prior$method_parameters$clinically_meaningful_diff[[1]]
      percentile_homogeneous <-
        self$prior$method_parameters$percentile_homogeneous[[1]]
      percentile_heterogeneous <-
        self$prior$method_parameters$percentile_heterogeneous[[1]]
      value_elastic_heterogeneous <-
        self$prior$method_parameters$value_elastic_heterogeneous[[1]]
      value_elastic_homogeneous <-
        self$prior$method_parameters$value_elastic_homogeneous[[1]]
      ...
      for (i in 1:num_simulations) {
        for (j in seq_along(target_treatment_effect_values)) {
          target_treatment_effect_estimate <- rnorm(
            1,
            target_treatment_effect_values[j],
            target_standard_error
          )

          pooled_variance <- ((self$source_sample_size_per_arm - 1) *
              self$source_data_variance + (target_sample_size_per_arm - 1) *
              target_data_variance
          ) / (self$source_sample_size_per_arm + target_sample_size_per_arm - 2)

          congruence_measures[i, j] <- max(
            self$source_sample_size_per_arm,
            ...
            abs(
                source_treatment_effect_estimate -
                  target_treatment_effect_estimate) / (
            sqrt(
              pooled_variance / self$source_sample_size_per_arm +
                pooled_variance / target_sample_size_per_arm
            )
          )
        }
      }
      quantile_homogeneous <- quantile(
        congruence_measures[, 1],
        percentile_homogeneous
      )
      quantile_heterogeneous1 <- quantile(
        congruence_measures[, 2],
        percentile_heterogeneous
      )
      quantile_heterogeneous2 <- quantile(
        congruence_measures[, 3],
        percentile_heterogeneous
      )
      ks_homogeneous <- quantile_homogeneous
      ks_heterogeneous <- min(quantile_heterogeneous1, quantile_heterogeneous2)
      b <- log((1 - value_elastic_homogeneous) * value_elastic_heterogeneous /
                 ((1 - value_elastic_heterogeneous) * value_elastic_homogeneous
                 )) /
        ((log(ks_homogeneous))^shape_param - (log(
                                                  ks_heterogeneous))^
                                                    shape_param)
      a <- log((1 - value_elastic_homogeneous) /
                 value_elastic_homogeneous) - b * (log(ks_homogeneous))^
        shape_param
      list(a = a, b = b)
    },
...
#' @field target_standard_error Standard error on the treatment effect in the
#'   target study
...
#' @export
#'
gaussian_elastic_prior_step <- R6::R6Class(
  ...
  public = list(
    ...
    initialize = function(prior) {
      ...
      # The implementation does not handle cases with different sample sizes in
      # each arm, therefore we compute an equivalent sample size per arm
      self$source_sample_size_per_arm <-
        self$prior$source$equivalent_source_sample_size_per_arm

      self$source_data_variance <- self$prior$source$standard_error^2 *
        self$source_sample_size_per_arm
    },
    ...
    #' @param target_standard_error Standard error of the treatment effect in
    #'   the target study
    #' @param target_treatment_effect_estimate Estimate of the treatment effect
    #'   in the target study
    method_calibration = function(target_sample_size_per_arm,
                                  target_standard_error,
                                  target_treatment_effect_estimate) {
      target_data_variance <- target_standard_error^2 *
        target_sample_size_per_arm # sample variance (estimate of the true sampling variance)
      ...
      self$congruence_threshold <- quantile(
        congruence_measures,
        self$prior$method_parameters$congruence_quantile[[1]]
      )
    },

    #' @description Generate distribution of congruence measure T in the
    #'   homogeneous case
    #'
    #' @param target_sample_size_per_arm Sample size per arm in the target study
    #' @param target_treatment_effect_estimate Treatment effect estimate in the
    #'   target study
    #' @param target_data_variance Estimated sampling variance of the target
    #'   data
    #' @return A numeric vector of congruence measures
    generate_congruence_measure_distribution =
      function(target_sample_size_per_arm,
               target_treatment_effect_estimate,
               target_data_variance) {
        ...
        for (i in 1:num_simulations) {
          pooled_variance <- ((source_sample_size_per_arm - 1) *
              source_data_variance + (target_sample_size_per_arm - 1) *
              target_data_variance
          ) / (source_sample_size_per_arm + target_sample_size_per_arm - 2)

          congruence_measures[i] <- max(
            ...
            abs(
                source_treatment_effect_estimate -
                  target_treatment_effect_estimate) / (
            sqrt(
              pooled_variance / source_sample_size_per_arm + pooled_variance /
                target_sample_size_per_arm
            )
          )
        }
