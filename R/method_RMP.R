...
#'
#' @keywords internal
mixture_variance <- function(w, sigma2_a, mu_a, sigma2_b, mu_b) {
  # Weighted sum of the variances
  weighted_var_sum <- w * sigma2_a + (1 - w) * sigma2_b

  # Weighted sum of the square of the means
  weighted_mean_sq_sum <- w * mu_a^2 + (1 - w) * mu_b^2

  # Square of the weighted sum of the means
  mean_weighted_sum_sq <- (w * mu_a + (1 - w) * mu_b)^2
...
#' @field info_prior_variance The variance of the informative prior
#'   distribution.
...
#' @field info_posterior_mean The mean of the informative posterior
#'   distribution.
#' @field vague_posterior_variance The variance of the vague posterior
#'   distribution.
#' @field info_posterior_variance The variance of the informative posterior
#'   distribution.
#' @field method Method name
#'
#' @export
gaussian_rmp_r_bes_t <- R6::R6Class(
  "GaussianRMP_RBesT",
  ...
  public = list(
      ...
      if (self$empirical_bayes) {
        self$vague_prior_variance <-
          (target_data$sample$treatment_effect_standard_error^
           2) *
          target_data$sample_size_per_arm # sample standard deviation in the target study

        # information provided by a single subject per arm in the target study
        # (as explained in the protocol)
        # Note that in Best et al, 2021 (Mepolizumab), this is the information
        # provided by a single subject in the target study
        ...
        # We use the following conditions to avoid bug in RBesT ELIR
        # computation if self$wpost == 0
        if (self$w == 1) {
        ...
        if (is.null(self$vague_prior_variance)) {
          stop(
               paste0("The vague prior variance must be defined",
                 " if empirical_bayes is False")) # information provided by a single subject per arm in the source study : Best et al (2023, case study on Belimumab)
        }
      }
    },
    ...
    posterior_moments = function(target_data) {
      ...
      # The following if statement is used because of a bug in the computation
      # of ELIR with RBesT that requires to define a single component of the
      # RMP if w = 0 or w = 1
      if (self$w == 0) {
        self$wpost <- 0
    ...
    prior_to_RBesT = function(...) {
      if (is.null(self$vague_prior_variance)) {
        stop(
          paste0("The vague prior variance must be",
            " defined. If empirical_bayes == TRUE,",
              " the prior can only be fully specified",
                " after the data has been observed.")
        )
      }
      ...
      # We use the following conditions to avoid bug in RBesT ELIR computation
      # if self$wpost == 0
      if (self$w == 1) {
        self$RBesT_prior <- RBesT::mixnorm(
          info = info
    ...
    posterior_to_RBesT = function(target_data, ...) {
      RBesT::sigma(self$RBesT_posterior) <-
  ...
  public = list(
    ...
    prior_weight = function(target_data) {
      ...
      if (is.null(sample_mean) || is.null(sample_size_per_arm)) {
        stop(
             paste0("One or more of the required variables",
               " (sample_mean, sample_size_per_arm) is", " NULL."))
      }
      ...
      log_likelihood_source <- (
        ...
        * (sample_mean - self$info_prior_mean)^2
        ...
        target_data$sample$treatment_effect_standard_error^2 +
          self$info_prior_variance
      )

      log_unnormalized_post_weights <- c(
        log(self$w) + log_likelihood_source,
        log(1 - self$w) + log_likelihood_vague
      )

      post_weights <- exp(
        log_unnormalized_post_weights - matrixStats::logSumExp(
                                                               log_unnormalized_post_weights)
      )
      ...
      if (is.na(wpost)) {
        stop(
          paste0("Posterior w is NaN, most likely due to",
            " both the vague and informative", " likelihood equal to 0.")
        )
      }
      self$wpost <- wpost
      self$posterior_parameters$prior_weight <- self$wpost
    ...
    #' @description  Calculate the prior probability density function (PDF) for
    ...
    posterior_moments = function(target_data) {
      ...
      vague_posterior_mean <- self$vague_prior_mean /
        (self$vague_prior_variance / standard_error^
           2 + 1) +
        sample_mean / (1
                       + standard_error^2
                       / self$vague_prior_variance)
      vague_posterior_variance <- 1 / (1 / self$vague_prior_variance + 1 /
                                         standard_error^
                                           2)

      info_posterior_mean <- self$info_prior_mean /
        (self$info_prior_variance / standard_error^
           2 + 1) + sample_mean /
        (1 +
           standard_error^2 /
             self$info_prior_variance)
      info_posterior_variance <- 1 / (1 / self$info_prior_variance + 1 /
                                        standard_error^
                                          2)
    ...
    posterior_mean = function() {
      (1 - self$wpost) * self$vague_posterior_mean + self$wpost *
        self$info_posterior_mean
    },
...
#'
#' @description This class represents a Bayesian borrowing model with a
#'   truncated Gaussian prior. It inherits from the MCMCModel
#'   class.
#'
...
#' @export
truncated_gaussian_rmp <- R6::R6Class(
  ...
  public = list(
    ...
    transformed parameters {
      ...
      real vague_normalizing_constant = normal_cdf(1-control_rate |
        vague_mean, vague_sd) - normal_cdf(-control_rate | vague_mean,
          vague_sd);
      real info_normalizing_constant = normal_cdf(1-control_rate | info_mean,
        info_sd) - normal_cdf(-control_rate | info_mean, info_sd);
    ...
    model {
      ...
      target += log_mix(w,
                normal_lpdf(target_treatment_effect | vague_mean,
                  vague_sd) - log_vague_normalizing_constant,
                normal_lpdf(target_treatment_effect | info_mean,
                  info_sd) - log_info_normalizing_constant);
    ...
    inference = function(target_data) {
      ...
      self$posterior_parameters$prior_weight <- self$w *
        prior_predictive_proba_vague / (self$w *
                                          prior_predictive_proba_vague +
                                          (1 - self$w) *
                                            prior_predictive_proba_info)
