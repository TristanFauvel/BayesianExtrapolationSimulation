export_table <- function(
  data_table, ncollapses = 1, column_names = NULL,
  title, file_path, longtable = TRUE, remake_tables = TRUE, page_width = 5
) {
  # Parameters:
  # page_width: Page width threshold in inches (default 6.5 inches for US
  # Letter with 1-inch margins)
  ...
  if (file.exists(pdf_file)) {
    ...
    if (length(crop_result) == 0) {
      warning(
              paste0("pdfcrop execution failed or returned no",
                " output. The PDF file might not be", " cropped correctly."))
    }
  } else {
    stop(
         paste0("The PDF file was not created",
           " successfully. Please check for LaTeX", " compilation errors."))
  }
}
