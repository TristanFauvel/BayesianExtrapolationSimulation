...
hellinger_distance <- function(mu1, sigma1, mu2, sigma2) {
  sqrt(1 - sqrt(2 * sigma1 * sigma2 / (sigma1^2 + sigma2^2)) * exp(-0.25 *
    (
     mu1 - mu2)^
      2 /
    (
...
compute_drift_range <- function(scenarios_config, case_study_config) {
  ...
  if (case_study_config$summary_measure_likelihood == "normal") {
    ...
    source_control_rate <- case_study_config$source$responses$control /
      case_study_config$source$control
    source_treatment_rate <- case_study_config$source$responses$treatment /
  ...
  drift_range <- seq(lower_bound, upper_bound,
    length.out =
      scenarios_config$ndrift
  )
  drift_range
}

#' Get important drift values for a given source treatment effect and case
#' study configuration.
#'
...
important_drift_values <- function(source_treatment_effect, case_study_config) {
...
#' Compute the source denominator range for a given source data, simulation
#' configuration, and case study configuration.
#'
...
compute_source_denominator_range <- function(source_data,
                                             scenarios_config,
                                             case_study_config) {
  # Compute, for each value of the source denominator, the corresponding
  # numerator so that the treatment effec stays the same
  ...
  if (source_data$endpoint == "binary" &&
    ...
    source_denominator_range <- source_data$control_rate *
      denominator_change_factor

    source_treatment_rate_range <- source_denominator_range *
      exp(source_data$treatment_effect_estimate)
  } else {
...
simulation_scenarios <- function(config_dir, scenarios_config) {
  ...
  # We store the parameters in a dataframe with two columns : method and
  # parameters
  for (method in names(methods_dict)) {
    if (method %in% scenarios_config$methods) {
      parameters_combinations <- combine_parameters(method)

      converted_parameters_df <- data.frame(
        method = method, parameters =
          I(apply(parameters_combinations, 1, function(row) {
      ...
      methods_configurations_df <- rbind(
        methods_configurations_df,
        converted_parameters_df
      )
    }
  }
  ...
  if (any(!(scenarios_config$methods %in% names(methods_dict)))) {
    stop(
         "Some methods listed in the simulation configuration need configuration.")
  }
  ...
  for (case_study in case_studies) {
    ...
    source_data <- observed_source_data$new(case_study_config)

    sampling_approximation <- case_study_config$sampling_approximation
    theta_0 <- case_study_config$theta_0
    null_space <- case_study_config$null_space

    source_denominator <- compute_source_denominator_range(
      source_data,
      scenarios_config, case_study_config
    )
    ...
    mandatory_drift_values <-
      important_drift_values(source_treatment_effect_estimate,
                             case_study_config)
    drift_range <- sort(c(drift_range, mandatory_drift_values))

    control_drift_range <-
      compute_control_drift_range(source_treatment_effect_estimate)
    ...
    source_denominator_repeated <- source_denominator[rep(seq_len(n),
      each =
        nrow(
             drift_combinations)
    ), ]
    drift_combinations_repeated <-
      drift_combinations[rep(seq_len(nrow(drift_combinations)), times = n), ]

    drift_combinations <- cbind(
      source_denominator_repeated,
      drift_combinations_repeated
    )
    ...
    total_target_sample_sizes <- (source_data$sample_size_control +
                                    source_data$sample_size_treatment) /
      scenarios_config$sample_size_factors
    ...
    for (i in seq_len(nrow(drift_combinations))) {
      ...
      source_denominator_change_factor <- drift_combinations[
        i,
      ...
      for (total_target_sample_size in total_target_sample_sizes) {
        ...
        current_scenario_configurations[[length(
                                                current_scenario_configurations) + 1]] <- list(
          ...
          target_to_source_std_ratio = drift_combinations[
            i,
    ...
    scenarios_configurations <- c(
      scenarios_configurations,
      current_scenario_configurations
    )
  }
  # Cartesian product between methods configurations and scenarios
  scenarios_configurations_df <- data.frame(do.call(
    rbind,
    scenarios_configurations
  ))
...
scenarios_table_ranges <- function(cases_df, results_path) {
  ...
  for (case_study in unique(cases_df$case_study)) {
    ...
    subdf$source_treatment_effect_estimate <-
      as.numeric(subdf$source_treatment_effect_estimate)
    ...
    df <- rbind(
      ...
      data.frame(
        Drug = paste0(
          toupper(substring(case_study, 1, 1)),
          substring(case_study, 2)
        ),
...
\\textbf{Case study} & \\textbf{Drift range} &
  \\textbf{Drift with $\\theta_T^{(true)} = \\theta_0$} &
    \\textbf{Treatment effect range} \\\\
\\midrule
"
    )
