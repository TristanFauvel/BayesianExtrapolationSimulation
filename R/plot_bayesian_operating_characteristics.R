...
#'
#' @description This function plots a metric against the parameters for a given
#'   method, case study, and sample size.
#'
...
#' @param target_sample_size_per_arm The target sample size per arm to plot the
#'   metric for.
#' @param theta_0 Boundary of the null hypothesis space.
#' @param add_baselines A logical value indicating whether to add baselines to
#'   the plot.
#'
#' @return None
#'
...
bayesian_metric_vs_parameters <- function(results_metrics_df,
  ...
  # Filter results_metrics_df on the selected case study, target sample arm and
  # method
  ...
  results_df[
    results_df[, "design_prior_type"] == "analysis_prior",
    "label"
  ] <- "Analysis prior"
  results_df[
    results_df[, "design_prior_type"] == "source_posterior",
    "label"
  ] <- "Source posterior"
  results_df[
    results_df[, "design_prior_type"] == "ui_design_prior",
    "label"
  ] <- "UI prior"

  # results_df <- results_df |>
  # filter(!is.na(prepost_proba_TP) & !is.na(average_tie) &
  # !is.na(average_power) & !is.na(prior_proba_success) &
  # !is.na(prior_proba_no_benefit) & !is.na(prior_proba_benefit) &
  # !is.na(prepost_proba_FP) & !is.na(upper_bound_proba_FP))
  ...
  for (i in seq_len(ncol(parameters_df))) {
    ...
    for (j in seq(n_params_loop)) {
      ...
      # Set a relative cap size (necessary, otherwise the cap size will depend
      # on the x-axis width)
      ...
      if (file.exists(paste0(file_path, ".pdf")) &&
            file.exists(paste0(file_path, ".png")) && remake_figures == FALSE) {
        return()
      }
...
#'
#' @description This function plots a metric against the sample size for a
#'   given method, case study, and parameters combinations.
#'
#' @param metric The metric to plot.
#' @param results_metrics_df The data frame containing the results and metrics.
#' @param case_study The case study to plot the metric for.
#' @param method The method to plot the metric for.
#' @param parameters_combinations The parameter combinations to filter the data
#'   on.
#' @param theta_0 Boundary of the null hypothesis space.
#' @param add_baselines A logical value indicating whether to add baselines to
#'   the plot.
#' @param target_to_source_std_ratio Ratio between the sampling standard
#'   deviation in the source study and in the
#'   target study.
#'
#' @return None
#'
...
bayesian_metric_vs_sample_size <- function(metric,
  ...
  # existing_columns <- columns_to_check[columns_to_check %in%
  # colnames(results_df)]
  #
  ...
  if (!(is.null(method))) {
    ...
    for (selected_method in unique(results_df$method)) {
      parameters <- get_parameters(results_df[results_df$method ==
      ...
      for (key in names(methods_dict[[selected_method]])) {
        ...
        if (is.null(methods_dict[[selected_method]][[key]][[
                                                            "important_values"]])) {
          important_parameters_values <-
            methods_dict[[selected_method]][[key]][["range"]]
        } else {
          important_parameters_values <-
            methods_dict[[selected_method]][[key]][["important_values"]]
        }
        if (selected_method == "commensurate_power_prior" && key ==
              "heterogeneity_prior") {
    ...
    # Process the row of a results dataframe to create a Method + Parameters
    # label
  ...
  if (!(is.null(method))) {
    if (metric %in% c("prior_proba_benefit", "prior_proba_no_benefit")) {
      # Only plot the prior probability of (no) benefit for the analysis design
      # prior.
      results_df <- results_df |>
        dplyr::filter(
          label == "Analysis prior"
      ...
      # Produce the plot
      design_priors_names <- c(
        "Analysis prior", "Source posterior",
        "UI design prior"
      )
    ...
    if (metric %in% c("prior_proba_benefit", "prior_proba_no_benefit") &&
          (design_prior != "analysis_prior")) {
      return(
      ) # The prior proba of (no) benefit is only relevant for an analysis prior.
    }
    # Produce the plot
    design_priors_names <- c(
      "Analysis prior", "Source posterior",
    ...
    plt <- ggplot2::ggplot(
      ...
      ggplot2::aes(
        x = target_sample_size_per_arm,
        y = !!selected_metric_name,
        group = factor(parameters_labels,
          levels =
            unique(results_df$parameters_labels)
        ),
        color = factor(parameters_labels,
          levels =
            unique(results_df$parameters_labels)
        ),
        shape = factor(parameters_labels,
          levels =
            unique(results_df$parameters_labels)
        )
      )
    ...
    plt <- plt +
      ...
      scale_shape_manual(
        values = seq_along(parameters_labels), labels =
          parameters_labels, guide = "legend"
      ) +
      guides(
        color = guide_legend(title = "Methods"), shape =
          guide_legend(title = "Methods")
      )
  }
...
bayesian_ocs_plots <- function(results_metrics_df, metrics) {
  ...
  design_prior_types <- c(
    "analysis_prior", "source_posterior",
    "ui_design_prior"
  )
  ...
  for (case_study in case_studies) {
    ...
    for (source_denominator_change_factor in unique(
                                                    results_df0$source_denominator_change_factor)) {
      ...
      for (target_to_source_std_ratio in unique(
                                                results_df1$target_to_source_std_ratio)) {
        ...
        for (method in methods) {
          ...
          parameters_combinations <- unique(get_parameters(results_df3[
            ,
            "parameters"
          ]))
          ...
          target_sample_sizes_per_arm <-
            unique(results_df3$target_sample_size_per_arm)
          ...
          for (metric in names(metrics)) {
            ...
            for (i in seq_len(nrow(parameters_combinations))) {
              # Plot the results with different design priors on the same plot,
              # for a single method
              bayesian_metric_vs_sample_size(
                ...
                case_study = case_study,
                method = method,
                parameters_combinations = parameters_combinations[i, ,
                  drop =
                    FALSE
                ],
                theta_0 = theta_0,
        ...
        for (metric in names(metrics)) {
          for (design_prior in design_prior_types) {
            # Plot the results for a single design priors for multiple methods
            # on the same plot
            bayesian_metric_vs_sample_size(
              metric = metric,
