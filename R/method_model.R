#' Model Class
#'
#' @description This class represents a statistical model for Bayesian
#'   borrowing analysis.
#' It provides methods for creating the model, performing inference, and
#' calculating posterior moments.
#'
#' @field empirical_bayes Logical indicating if empirical Bayes method is used.
#' @field analytic_ocs Results of the analytic operating characteristics
#'   simulation.
...
#' @field RBesT_posterior_normix Normal mixture approximation to the posterior
#'   distribution
#' @field RBesT_prior_normix Normal mixture approximation to the prior
#'   distribution
#' @field summary_measure_likelihood Likelihood familly, that is, the
#'   distribution used to model the summary
#'   measure of the treatment effect.
#' @field n_components_mixture_approx Number of mixture components used for the
#'   mixture approximation
#' @field aic_penalty_parameter_mixture_approx AIC penalty parameter used for
#'   the mixture approximation
#' @export
model <- R6::R6Class(
  "Model",
  public = list(
    ...
    #'
    #' @description This method creates a Model object based on the specified
    ...
    create = function(case_study_config,
      ...
      if (method == "RMP") {
        ...
        if (case_study_config$summary_measure_likelihood == "normal") {
          model <- gaussian_rmp_r_bes_t$new(prior = prior)
        } else if (case_study_config$summary_measure_likelihood == "binomial") {
          model <- truncated_gaussian_rmp$new(
            prior = prior, mcmc_config =
              mcmc_config
          )
        } else {
          stop(
               paste0("Only \"normal\" or \"binomial\" treatment",
                 " effect distributions are supported"))
        }
      } else if (method == "separate") {
        if (case_study_config$summary_measure_likelihood == "normal") {
          model <- separate_gaussian_r_bes_t$new(prior = prior)
        } else if (case_study_config$summary_measure_likelihood == "binomial") {
          model <- binomial_separate$new(
            prior = prior, mcmc_config =
              mcmc_config
          )
        } else {
        ...
        if (case_study_config$summary_measure_likelihood == "normal") {
          model <- pool_gaussian_r_bes_t$new(prior = prior)
        } else if (case_study_config$summary_measure_likelihood == "binomial") {
          model <- binomial_pooling$new(prior = prior, mcmc_config =
            mcmc_config)
        } else {
          stop(
               paste0("Only \"normal\" or \"binomial\" treatment",
        ...
        if (case_study_config$summary_measure_likelihood == "normal") {
          model <- static_borrowing_gaussian$new(prior = prior)
        } else if (case_study_config$summary_measure_likelihood == "binomial") {
          model <- binomial_cpp$new(prior = prior, mcmc_config = mcmc_config)
        } else {
          stop(
               paste0("Only \"normal\" or \"binomial\" treatment",
                 " effect distributions are supported"))
        ...
        if (case_study_config$summary_measure_likelihood == "normal") {
          model <- test_then_pool_equivalence$new(prior = prior)
        } else if (case_study_config$summary_measure_likelihood == "binomial") {
          model <- test_then_pool_equivalence$new(
            prior = prior, mcmc_config =
              mcmc_config
          )
        }
        ...
        if (case_study_config$summary_measure_likelihood == "normal") {
          model <- test_then_pool_difference$new(prior = prior)
        } else if (case_study_config$summary_measure_likelihood == "binomial") {
          model <- test_then_pool_difference$new(
            prior = prior, mcmc_config =
              mcmc_config
          )
        }
      } else if (method == "PDCCPP") {
        model <- pdccpp$new(prior,
          theta_0 = case_study_config$theta_0,
          null_space = null_space
        )
      } else if (method == "EB_PP") {
        model <- gaussian_gravestock_ebpp$new(prior,
          null_space = null_space,
          theta_0 = case_study_config$theta_0
        )
      } else if (method == "NPP") {
        ...
        if (case_study_config$summary_measure_likelihood == "normal") {
          model <- gaussian_commensurate_power_prior$new(
            prior = prior,
            mcmc_config = mcmc_config
          )
        } else if (case_study_config$summary_measure_likelihood == "binomial") {
        ...
        if (case_study_config$summary_measure_likelihood == "normal") {
          model <- gaussian_elastic_prior_step$new(prior = prior)
        }
      } else if (method == "GaussianElasticPriorLogistic") {
        if (case_study_config$summary_measure_likelihood == "normal") {
          model <- gaussian_elastic_prior_logistic$new(prior = prior)
        }
      } else {
    ...
    #' @description Abstract method to calculate the cumulative distribution
    ...
    prior_cdf = function(target_treatment_effect) {
      ...
      for (i in seq(1, length(target_treatment_effect))) {
        ...
        # The integrate function has bad behavior for large upper bound, with a
        # non-monotonic behavior despite a positive integrand. The following
        # decomposition solves the issue:
      ...
      # The value of the CDF may be inaccurate for large x values due to double
      # integration.
      integral
    },
    ...
    #' @param n_samples_quantiles_estimation Number of samples used to
    #'   estimates quantiles of
    #'   distributions
    #' @return The meen of the posterior distribution
    posterior_mean = function(mcmc, n_samples_quantiles_estimation = NA) {
      if (mcmc == TRUE) {
        posterior_samples <-
          self$sample_posterior(n_samples_quantiles_estimation)
    ...
    posterior_quantile = function(p, mcmc, n_samples_quantiles_estimation =
                                    NA) {
      if (mcmc == TRUE) {
        posterior_samples <-
          self$sample_posterior(n_samples_quantiles_estimation)
        stats::quantile(posterior_samples, probs = p)
      } else {
        res <- c()
        for (pi in p) {
          root <- stats::uniroot(
    ...
    #' Calculates the credible interval of the posterior distribution for a
    #' given level
    #'
    ...
    #' @description Method to calculate the effective sample size of the prior
    #'   distribution
    prior_ESS = function() {
      stop("Not implemented", call. = FALSE)
    },
    ...
    test_decision = function(critical_value,
                             ...
                             confidence_level) {
      if (self$mcmc == TRUE) {
        if (critical_value != (1 + confidence_level) / 2) {
          stop(
            paste0(
              "Only the 97.5 and 2.5 percentiles are ",
              "computed, so you can only consider "
            )
          )
        }
        percentile_97_5 <- self$credible_interval_97.5
        percentile_2_5 <- self$credible_interval_2.5

        if (null_space == "left") {
          percentile_2_5 > theta_0
        } else {
          percentile_97_5 < theta_0
        }
      } else if (!is.null(self$posterior_summary)) {
        if (null_space == "left") {
          self$posterior_summary["cri95L"] > theta_0
        } else {
          self$posterior_summary["cri95U"] < theta_0
        }
      } else {
        if (null_space == "left") {
          1 - self$posterior_cdf(theta_0) > critical_value
        } else {
          self$posterior_cdf(theta_0) > critical_value
        }
      }
    },
    ...
    #' @return A list of simulation results including test decisions, posterior
    #'   means, medians, credible intervals, and posterior parameters
    simulation_for_given_treatment_effect = function(target_data,
      ...
      test_decisions <-
        if (requested("test_decision")) numeric(n_replicates) else NULL
      posterior_means <-
        if (requested("posterior_mean")) numeric(n_replicates) else NULL
      posterior_medians <-
        if (requested("posterior_median")) numeric(n_replicates) else NULL
      ...
      ess_moments <-
        if (requested("ess_moment")) numeric(n_replicates) else NULL
      ess_precisions <-
        if (requested("ess_precision")) numeric(n_replicates) else NULL
      ess_elir <- if (requested("ess_elir")) numeric(n_replicates) else NULL
      posterior_parameters <- NULL
      fit_success <-
        if (requested("fit_success")) character(n_replicates) else NULL
      mcmc_ess <-
        if (requested("mcmc_diagnostics")) numeric(n_replicates) else NULL
      ...
      cleanup_stan_draws <- function() {
        ...
        file_ages <- as.numeric(difftime(Sys.time(), creation_times,
          units =
            "secs"
        ))
        ...
        while (retry) {
          ...
          if (requested("posterior_median")) {
            if (is.null(self$post_median)) {
              posterior_medians[r] <-
                self$posterior_median(n_samples_quantiles_estimation)
            } else {
          ...
          if (requested("credible_interval")) {
            credible_intervals[r, ] <- self$credible_interval(
              level =
                confidence_level
            )
          }
          ...
          if (requested("ess_moment") || requested("ess_precision")) {
            ...
            if (requested("ess_moment")) {
              ess_moments[r] <- prior_moment_ess(
            ...
            if (requested("ess_precision")) {
              ess_precisions[r] <- prior_precision_ess(
                rbest_model = self$RBesT_posterior_normix,
                target_data = target_data
              )
            }
          ...
          if (requested("ess_elir")) {
            if (r == 1 || self$empirical_bayes == TRUE) {
              # Only Empirical Bayes Methods have a prior that varies from on
          ...
          if (requested("mcmc_diagnostics") && self$mcmc) {
            # If the target ESS is not reached, start the iteration again,
            # increasing the chain length, unless it is already at the maximal
            # value allowed, which is 3x the required ESS.
            # If the target ESS is reached by a large factor, decrease the
            # chain length and proceed to the next iteration.
            ...
            if (1.1 < factor) {
              ...
              self$mcmc_config$chain_length <- as.integer(1.1 *
                                                            self$mcmc_config$chain_length / factor)
            } else if (factor < 1) {
              # The target MCMC ESS is not reached
              # Increase the chain lengths and start the iteration again
              new_length <- as.integer(1.1 * self$mcmc_config$chain_length /
                                         factor)
          ...
          if (verbose == 1) {
            print(paste0(
              "Duration of iteration ", r, " is ", time_taken,
              " seconds."
            ))
          }

          # Avoid scanning the draws directory after every replicate. Files
          # cannot
          # become eligible for age-based cleanup more often than once per
          # minute.
    ...
    #' @description Method to calculate the prior probability of treatment
    #'   benefit
    #' @param theta_0 Null hypothesis value
    #' @param null_space The null space for hypothesis testing
    ...
    #' @param method Method name
    #' @param verbose Verbosity level (0 or 1)
    #' @return A list of estimated frequentist operating characteristics
    #'   including coverage, MSE, bias, posterior mean, median,
    #'   precision, credible intervals, and success probability
    estimate_frequentist_operating_characteristics = function(theta_0,
      ...
      # Determine whether the true value of the target treatment effect lies
      # within the credible interval (to compute the coverage)
      estimate_in_cr_i <- (
        credible_intervals[, 1] <= target_treatment_effect &
          target_treatment_effect <= credible_intervals[, 2]
      )

      coverage <- mean(estimate_in_cr_i)

      conf_int_coverage <- binom.test(sum(estimate_in_cr_i),
        length(estimate_in_cr_i),
      ...
      if (!is.null(posterior_parameters)) {
        for (parameter in colnames(posterior_parameters)) {
          posterior_params[[parameter]] <-
            mean(posterior_parameters[[parameter]])

          if (n_replicates >= 10000) {
            ci <- Hmisc::smean.cl.normal(posterior_parameters[[parameter]],
              conf.int = confidence_level
            )
          } else {
            ci <- Hmisc::smean.cl.boot(posterior_parameters[[parameter]],
              conf.int = confidence_level
            )
          }
      ...
      conf_int_proba_success <- binom.test(sum(test_decisions),
        length(test_decisions),
        conf.level = confidence_level
      )$conf.int
      mcse_proba_success <- sqrt(proba_success * (1 - proba_success) /
                                   n_replicates)
      ...
      if (n_replicates >= 1000) {
        conf_int_precision <- Hmisc::smean.cl.normal(half_widths,
          conf.int =
            confidence_level
        )[2:3]
        conf_int_ess_moment <- Hmisc::smean.cl.normal(ess_moments,
          conf.int =
            confidence_level
        )[2:3]
        conf_int_ess_precision <- Hmisc::smean.cl.normal(ess_precisions,
          conf.int = confidence_level
        )[2:3]
        conf_int_ess_elir <- Hmisc::smean.cl.normal(results$ess_elir,
          conf.int = confidence_level
        )[2:3]
        conf_int_mse <- Hmisc::smean.cl.normal(squared_errors,
          conf.int =
            confidence_level
        )[2:3]
        conf_int_bias <- Hmisc::smean.cl.normal(errors,
          conf.int =
            confidence_level
        )[2:3]
        conf_int_posterior_mean <- Hmisc::smean.cl.normal(posterior_means,
          conf.int = confidence_level
        )[2:3]
        conf_int_posterior_median <- Hmisc::smean.cl.normal(posterior_medians,
          conf.int = confidence_level
        )[2:3]
        conf_int_rhat <- Hmisc::smean.cl.normal(rhat_values,
          conf.int =
            confidence_level
        )[2:3]
        conf_int_mcmc_ess <- Hmisc::smean.cl.normal(mcmc_ess_values,
          conf.int = confidence_level
        )[2:3]
        conf_int_n_divergences <- Hmisc::smean.cl.normal(n_divergences_values,
          conf.int = confidence_level
        )[2:3]
      } else {
        conf_int_precision <- Hmisc::smean.cl.boot(half_widths,
          conf.int =
            confidence_level
        )[2:3]
        conf_int_ess_moment <- Hmisc::smean.cl.boot(ess_moments,
          conf.int =
            confidence_level
        )[2:3]
        conf_int_ess_precision <- Hmisc::smean.cl.boot(ess_precisions,
          conf.int = confidence_level
        )[2:3]
        conf_int_ess_elir <- Hmisc::smean.cl.boot(results$ess_elir,
          conf.int =
            confidence_level
        )[2:3]
        conf_int_mse <- Hmisc::smean.cl.boot(squared_errors,
          conf.int =
            confidence_level
        )[2:3]
        conf_int_bias <- Hmisc::smean.cl.boot(errors,
          conf.int =
            confidence_level
        )[2:3]
        conf_int_posterior_mean <- Hmisc::smean.cl.boot(posterior_means,
          conf.int = confidence_level
        )[2:3]
        conf_int_posterior_median <- Hmisc::smean.cl.boot(posterior_medians,
          conf.int = confidence_level
        )[2:3]
        conf_int_rhat <- Hmisc::smean.cl.boot(rhat_values,
          conf.int =
            confidence_level
        )[2:3]
        conf_int_mcmc_ess <- Hmisc::smean.cl.boot(mcmc_ess_values,
          conf.int =
            confidence_level
        )[2:3]
        conf_int_n_divergences <- Hmisc::smean.cl.boot(n_divergences_values,
          conf.int = confidence_level
        )[2:3]
      }
    ...
    #' @param target_to_source_std_ratio Ratio between the target and source
    ...
    estimate_bayesian_operating_characteristics = function(design_prior,
                                                           ...
                                                           method,
                                                           n_samples_quantiles_estimation) {
      test_decisions <- matrix(rep(0, n_samples_design_prior * n_replicates),
        nrow = n_samples_design_prior
      )
      ...
      for (i in seq_along(design_prior_samples)) {
        drift <-
          design_prior_samples[
                               i] -
                                 source_data$treatment_effect_estimate # This implies that target_data$treatment_effect <- design_prior_samples[i]
      ...
      conditional_proba_success <-
        rowMeans(test_decisions) # Contains Pr(Study success | theta_T)
      ...
      average_tie_estimate <- average_tie(
        prepost_proba_fp_estimate,
        prior_proba_no_benefit_estimate
      )

      average_power_estimate <- average_power(
        prepost_proba_tp_estimate,
        prior_proba_no_benefit_estimate
      )
    ...
    #' @param n_samples_mixture_approx Number of samples used to approximate
    #'   the prior with a mixture.
    prior_to_RBesT = function(n_samples_mixture_approx) {
      # Note that this function is overriden in models for which there is no
      # need to sample the prior and posterior to get the mixture
      # approximation.

      prior_samples <- self$sample_prior(
        n_samples =
          n_samples_mixture_approx
      ) # Samples to be fitted by a mixture distribution
      ...
      if (self$summary_measure_likelihood == "normal") {
        ...
        transformed_samples <- (prior_samples + 1) /
          2 # Transformed samples in the [0, 1] range
      ...
      # RBesT::sigma(prior_mixture_approximation) <- target_standard_deviation
      # # Set the reference scale
      # }
    ...
    posterior_to_RBesT = function(target_data, simulation_config) {
      ...
      posterior_samples <-
        self$sample_posterior(
                              simulation_config$n_samples_mixture_approx) # Samples to be fitted by a mixture
      ...
      if (self$summary_measure_likelihood == "normal") {
        ...
        RBesT::sigma(posterior_mixture_approximation) <-
          target_data$sample$standard_deviation # Set the reference scale

        self$RBesT_posterior_normix <- posterior_mixture_approximation
      } else if (self$summary_measure_likelihood == "binomial") {
        transformed_samples <- (posterior_samples + 1) /
          2 # Transformed samples in the [0, 1] range
        # The posterior approximation with a mixture of Beta distributions is
        # based on the non-transformed samples.
        posterior_mixture_approximation <- RBesT::automixfit(
          transformed_samples,
          Nc = self$n_components_mixture_approx,
          k = self$aic_penalty_parameter_mixture_approx,
        ...
        # The posterior approximation with a mixture of Gaussians is based on
        # the non-transformed samples.
        ...
        RBesT::sigma(self$RBesT_posterior_normix) <-
          target_data$sample$standard_deviation # Set the reference scale
      } else {
        stop("Distribution not supported")
      }
    ...
    plot_pdfs = function(xmin = -2,
      ...
      plt <- ggplot(df_long, aes(x = x, y = value, color = variable)) +
        ...
        ggplot2::labs(
          title =
            "Prior and posterior distribution of the treatment effect", x =
            "Treatment effect", y = "Density"
        ) +
        scale_color_manual(
          values = c(
            "prior_pdf" = "blue",
          ...
          labels = c(
            "prior_pdf" = "Prior pdf", "posterior_pdf" =
              "Posterior pdf"
          )
        ) +
        theme_minimal() +
...
#'
#' @description This class represents a conjugate Gaussian model (Gaussian
...
conjugate_gaussian <- R6::R6Class(
  ...
  public = list(
    ...
    #' @param target_treatment_effect Point at which to evaluate the posterior
    #'   CDF
    ...
    posterior_mean = function(target_data) {
      post_mean <- (
        self$prior_mean / (
          self$prior_var /
            target_data$sample$treatment_effect_standard_error^2 + 1
        )
      ) + (
        target_data$sample$treatment_effect_estimate / (
          1 + target_data$sample$treatment_effect_standard_error^2 /
            self$prior_var
        )
      )
    ...
    prior_to_RBesT = function(...) {
      ...
      self$RBesT_prior <- RBesT::mixnorm(dist = c(
        1, self$prior_mean,
        sqrt(self$prior_var)
      ))
...
#'
#' @description This class represents a model with a Gaussian prior derived
#'   from static borrowing, and a Gaussian likelihood. It
#'   inherits from the `ConjugateGaussian` class.
#'
#' @field power_parameter The power parameter for the model. A NULL value
#'   indicates no power parameter, whereas a non-zero
#'   value sets the prior variance based on the source's
#'   standard error and the power parameter.
#' @field prior_var The variance of the prior. This is set based on the power
#'   parameter.
#' @field method Method name
#' @examples NA
static_borrowing_gaussian <- R6::R6Class(
  "StaticBorrowingGaussian",
  inherit = conjugate_gaussian,
  public = list(
...
#'
#' @description This class represents a model with separate Gaussian
#'   components, inheriting from `StaticBorrowingGaussian`. The
#'   power parameter is set to 0.
#' @field method Method name
#' @examples NA
separate_gaussian <- R6::R6Class(
  "SeparateGaussian",
  inherit = static_borrowing_gaussian,
  public = list(
    method = "separate",
    #' @param prior Prior
    #' @return A Model object.
...
#' PoolGaussian class
#'
#' @description This class represents a model with pooled Gaussian components,
#'   inheriting from `StaticBorrowingGaussian`. The power
#'   parameter is set to 1.
#' @field method Method name
#' @examples NA
pool_gaussian <- R6::R6Class(
  "PoolGaussian",
  inherit = static_borrowing_gaussian,
  public = list(
    method = "gaussian",
...
model_r_bes_t <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description Calculates the prior probability density function (PDF) for
    #'   a given target treatment effect.
    #' @param target_treatment_effect The target treatment effect.
    #' @return The prior PDF.
    prior_pdf = function(target_treatment_effect) {
      RBesT::dmix(self$RBesT_prior, target_treatment_effect, log = FALSE)
    },

    #' @description Calculates the prior cumulative distribution function (CDF)
    #'   for a given target treatment effect.
    #'
    #' @param target_treatment_effect The target treatment effect.
    ...
    posterior_moments = function(target_data) {
      ...
      names(inference_results) <- c(
        "mean", "standard_deviation", "cri95L",
        "median", "cri95U"
      )
    ...
    posterior_variance = function() {
      ...
      self$posterior_summary <- inference_results
      self$posterior_summary["standard_deviation"]^
        2
    },
    ...
    #' @description Calculates the posterior probability density function (PDF)
    #'   for a given target treatment effect.
    #'
    #' @param target_treatment_effect The target treatment effect.
    #' @return The posterior PDF.
    posterior_pdf = function(target_treatment_effect) {
      RBesT::dmix(self$RBesT_posterior, target_treatment_effect, log = FALSE)
    },

    #' @description Calculates the posterior cumulative distribution function
    #'   (CDF) for a given target treatment effect.
    #'
    #' @param target_treatment_effect The target treatment effect.
    ...
    #' @param level Level of the credible interval.
    #' @return A vector containing the lower and upper bounds of the credible
    #'   interval.
    credible_interval = function(level = 0.95) {
      if (level != 0.95) {
        stop("Not implemented for other than 95% CrI")
      }
      c(self$posterior_summary["cri95L"], self$posterior_summary["cri95U"])
    }
  )
)
...
#' @field method Method name
#' @export
separate_gaussian_r_bes_t <- R6::R6Class(
  "SeparateGaussian_RBesT",
  inherit = model_r_bes_t,
  public = list(
    method = "separate",
    ...
    initialize = function(prior) {
      ...
      self$prior_mean <- prior$source$treatment_effect_estimate
      self$prior_var <- 1000 # Vague prior
      self$RBesT_prior <- RBesT::mixnorm(c(
        1, self$prior_mean,
        sqrt(self$prior_var)
      ))
    ...
    #' @return None
    posterior_to_RBesT = function(target_data, ...) {
      self$RBesT_posterior <- RBesT::mixnorm(c(
        1, self$post_mean,
        sqrt(self$post_var)
      ), sigma = target_data$sample$standard_deviation)
      self$RBesT_posterior_normix <- self$RBesT_posterior
...
#' @field method Method name
#' @export
pool_gaussian_r_bes_t <- R6::R6Class(
  "PoolGaussian_RBesT",
  inherit = model_r_bes_t,
  public = list(
    method = "pooling",
...
#' MCMCModel class
#'
#' @description This class represents a Bayesian borrowing model using MCMC
#'   sampling.
...
#' @field treatment_effect_summary Summary statistics of the treatment effect
...
#' @field prior_pdf_approx Approximation to the prior probability density
#'   function
#' @field prior_cdf_approx Approximation to the prior cumulative density
#'   function
#' @field posterior_pdf_approx Approximation to the posterior probability
#'   density function
#' @field posterior_cdf_approx Approximation to the posterior cumulative
#'   density function
#'
#' @export
mcmc_model <- R6::R6Class(
  "MCMCModel",
  inherit = model,
  public = list(
    stan_model = NULL,
    ...
    prepare_data = function(target_data) {
      stop("Subclasses must implement the 'prepare_data' method.",
        call. =
          FALSE
      )
    },
    ...
    inference = function(target_data) {
      ...
      self$treatment_effect_summary <-
        treatment_effect_fit_summary[treatment_effect_fit_summary$variable ==
                                     "target_treatment_effect", ]
      ...
      if (mcmc_ess[["target_treatment_effect"]] < self$mcmc_config$target_ess) {
        return(paste0(
          "Target MCMC ESS not reached, ESS is : ",
          mcmc_ess[["target_treatment_effect"]]
        )) # Store as warning in the results table
      }

      if (rhat_values[
                      "target_treatment_effect"] >
                        self$mcmc_config$rhat_threshold) {
        return(paste0(
          "Large rhat values: ",
          rhat_values["target_treatment_effect"]
        )) # Store as warning in the results table
      }
      ...
      self$posterior_pdf_approx <-
        density(treatment_effect_draws) # Kernel Density Estimation
    ...
    credible_interval = function(level = 0.95) {
      ...
      ci <- c(
        self$treatment_effect_summary$q2.5,
    ...
    #' @param n_samples The number of samples to draw from the posterior
    #'   distribution
    ...
    #' @description Compute the posterior parameters. If there are posterior
    ...
    posterior_pdf = function(target_treatment_effect) {
      ...
      pdf_x <- approx(self$posterior_pdf_approx$x,
        self$posterior_pdf_approx$y,
        xout = target_treatment_effect
      )$y
      pdf_x
    },
    #' @description Calculates the posterior cumulative distribution function
    ...
    #' @description Prior PDF
    #' @param target_treatment_effect Point at which to evaluate the prior PDF
    #' @param n_samples_quantile_estimation Number of samples used to estimate
    #'   the quantiles of the distribution
    prior_pdf = function(target_treatment_effect,
                         n_samples_quantile_estimation = 10000) {
      if (is.null(self$prior_pdf_approx)) {
        treatment_effect_draws <-
          self$sample_prior(n_samples_quantile_estimation)
        self$prior_pdf_approx <-
          density(treatment_effect_draws) # Kernel Density Estimation
      }
      pdf_x <- approx(self$prior_pdf_approx$x, self$prior_pdf_approx$y,
        xout =
          target_treatment_effect
      )$y
      pdf_x
    ...
    sample_prior = function(n_samples) {
      ...
      treatment_effect_draws <-
        self$prior_draws$draws("target_treatment_effect")
      treatment_effect_draws <- unlist(as.list(treatment_effect_draws))
      return(sample(
        treatment_effect_draws,
        size = n_samples,
...
#'
#' @description This class represents a Truncated Gaussian model using the CPP
#'   (Conditional Power Prior) approach with power parameter
#'   set to 0.
#' It inherits from the BinomialCPP class.
#' @field method Method name
#' @export
binomial_separate <- R6::R6Class(
  "BinomialSeparate",
  inherit = mcmc_model,
  public = list(
    method = "separate",
    ...
    initialize = function(prior, mcmc_config) {
      ...
      self$stan_model_code <- "
        ...
        parameters {
          ...
          real<lower = - control_rate, upper =
            1 - control_rate> target_treatment_effect;
        }
      ...
      data_list <- list(
        ...
        n_successes_treatment = as.integer(
          target_data$sample_size_treatment *
            target_data$sample$sample_treatment_rate
        ),
        n_successes_control = as.integer(
          target_data$sample_size_control *
            target_data$sample$sample_control_rate
        )
      )
      data_list
    },
    ...
    prepare_data = function(target_data) {
      ...
      data_list <- list(
        ...
        n_control = as.integer(
          target_data$sample_size_control +
            self$prior$source$sample_size_control
        ),
        n_successes_treatment = as.integer(
          target_data$sample_size_treatment *
            target_data$sample$sample_treatment_rate +
        ...
        n_successes_control = as.integer(
          target_data$sample_size_control *
            target_data$sample$sample_control_rate +
            self$prior$source$sample_size_control *
              self$prior$source$control_rate
        )
      )
      data_list
    },
