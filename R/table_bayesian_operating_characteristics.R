...
#' @return This function doesn't return a value but saves the generated tables
#'   as HTML, PDF, and LaTeX files.
#'
#' @import dplyr
#' @import knitr
#' @export
table_bayesian_metrics <- function(results_metrics_df,
  ...
  results_df <- results_df |> select(-c(
    "case_study", "method", "parameters",
    "target_sample_size_per_arm", "source_denominator_change_factor",
    "target_to_source_std_ratio", "source_treatment_effect_estimate",
    "source_standard_error", "endpoint", "summary_measure_likelihood",
  ...
  for (i in seq_along(bayesian_metrics)) {
    ...
    if (metric_name == "prior_proba_benefit" || metric_name ==
          "prior_proba_no_benefit") {
      # Select the metric of interest
      results_df <- results_df[results_df$design_prior_type ==
                                 "Analysis prior", ]
      ...
      if (metric_name == "prior_proba_benefit") {
        colnames(data_wide)[colnames(data_wide) ==
                              metric_name] <- "Prior probability of benefit"
      } else if (metric_name == "prior_proba_no_benefit") {
        colnames(data_wide)[colnames(data_wide) ==
                              metric_name] <- "Prior probability of no benefit"
      }
    } else {
      # Select the metric of interest
      data_table <- results_df |> select(c(
        "Method", "design_prior_type",
        metric_name
      ))
    ...
    export_table(
      data_table = data_wide, ncollapses = 1, title = title,
      file_path = file_path
    )
  }
}
...
table_bayesian_metrics_sample_sizes_comparison <- function(results_metrics_df,
  ...
  results_df <- results_df |> select(-c(
    ...
    "source_treatment_effect_estimate", "source_standard_error", "endpoint",
    "summary_measure_likelihood", "source_sample_size_control",
  ...
  for (i in seq_along(bayesian_metrics)) {
    ...
    wide_data_table <- data_table |>
      pivot_wider(
        names_from = target_sample_size_per_arm,
        # Columns become values of `target_sample_size_per_arm`
    ...
    export_table(
      data_table = wide_data_table_filtered, ncollapses = 1,
...
table_bayesian_metrics_across_case_studies <- function(results_metrics_df) {
  ...
  for (i in seq_along(bayesian_metrics)) {
    ...
    # title <- format_title(title = title, case_study = NA,
    # target_to_source_std_ratio = target_to_source_std_ratio,
    # source_denominator_change_factor = source_denominator_change_factor,
    # as_latex = TRUE)
    filename <- format_filename(
      filename = filename, case_study = NA,
      target_to_source_std_ratio = target_to_source_std_ratio,
      source_denominator_change_factor = source_denominator_change_factor
    )
...
#'
#' This function creates tables for various Bayesian operating characteristics
#' across different case studies and methods.
#'
#' @param results_metrics_df A data frame containing the results and metrics.
#' @param metrics A vector of metric names to include in the tables.
#'
#' @return This function doesn't return a value but calls other functions to
...
#'
#' @export
table_bayesian_ocs <- function(
  results_metrics_df, metrics,
  source_denominator_change_factor = 1,
  target_to_source_std_ratio = 1
) {
  ...
  for (case_study in case_studies) {
    ...
    target_sample_sizes_per_arm <-
      unique(results_df$target_sample_size_per_arm[results_df$case_study ==
                                                     case_study])

    for (source_denominator_change_factor in unique(
                                                    results_df$source_denominator_change_factor)) {
      for (target_to_source_std_ratio in unique(
                                                results_df$target_to_source_std_ratio)) {
