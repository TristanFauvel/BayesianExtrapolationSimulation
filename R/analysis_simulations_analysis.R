...
#'
#' @description This function performs a simulation analysis for a given
#'   environment using the specified
#' analysis configuration and configuration directory. It reads the output
#' configuration,
#' loads results, applies frequentist power analysis, and computes the sweet
#' spot.
#'
#' @param env A character string specifying the environment for the analysis
#'   (e.g., "development", "production").
#' @param analysis_config A list containing the analysis configuration
#'   parameters.
#' @param config_dir A character string specifying the directory containing the
#'   configuration files.
#' @param frequentist_metrics List of frequentist metrics.
#'
#' @return This function does not return a value. It writes the results and
#'   sweet spot analysis
...
simulation_analysis <- function(env,
                                analysis_config,
                                config_dir,
                                frequentist_metrics, case_studies = "all",
                                to_compute =
                                  c(
                                    "frequentist_power_at_equivalent_tie",
                                    "frequentist_power_at_nominal_tie",
                                    "sweet_spot", "bayesian_ocs"
                                  ),
                                methods = "all") {
  futile.logger::flog.info(
                           "Starting the analysis of results in environment %s", env)
  ...
  simulation_config <-
    yaml::read_yaml(system.file("conf/simulation_config.yml", package =
                                  "RBExT"))
  ...
  if ("frequentist_power_at_equivalent_tie" %in% to_compute) {
    new_results_power_df <- frequentist_power_at_equivalent_tie(
      results =
        results_freq_subset, analysis_config = analysis_config,
      simulation_config = simulation_config, parallelization = FALSE
    )
    ...
    results_freq_df <- dplyr::bind_rows(
      updated_results_df,
      new_results_power_df
    )
  ...
  if ("frequentist_power_at_nominal_tie" %in% to_compute) {
    new_results_power_df <- frequentist_power_at_nominal_tie(
      results =
        results_freq_subset, analysis_config = analysis_config,
  ...
  if ("bayesian_ocs" %in% to_compute) {
    ...
    if (file.exists(bayes_filename)) {
      ...
      # Remove rows in `existing_bayesian_df` that match the keys in
      ...
      combined_bayesian_df <- dplyr::bind_rows(
        updated_bayesian_df,
        results_bayesian_ocs
      )
