    ...
    if (!(labels_title == "Parameters")) {
      ...
      if (length(unique(data$parameters)) > 1) {
        plt <- plt +
          ...
          scale_color_viridis_d(labels = parameters_labels) + labs(
          color =
            labels_title
        )
...
#' @param category The category to group the results by (either "parameters" or
#'   "target_sample_size_per_arm")
#' @param control_drift Logical indicating whether to filter results by control
#'   drift (TRUE) or treatment drift (FALSE)
#' @param target_sample_size_per_arm The target sample size per arm
#' @param parameters_combinations The combinations of parameters to filter the
#'   results by
...
#'   studies sampling standard deviation.
...
plot_metric_vs_drift <- function(metric,
  ...
  if (category == "target_sample_size_per_arm") {
    ...
    labels_title <-
      latex2exp::TeX(
                     "$\\sigma_T/\\sigma_S$") # Ratio between target and source standard deviation
  } else {
  ...
  for (i in seq_along(categories)) {
    ...
    if (category == "source_denominator_change_factor") {
      if (is.na(combination)) {
        ...
        results_df[results_df[, "source_denominator_change_factor"] ==
  ...
  if (metric %in% c("tie", "success_proba")) {
    plt <- plt +
      ggplot2::geom_hline(
        yintercept = analysis_config$nominal_tie, color =
...
plot_metric_vs_parameters <- function(results_metrics_df,
  ...
  color_map <-
    viridis::viridis(length(unique(results_df$target_treatment_effect)))
  ...
  # Loop over the different parameters that take different values in the
  # simulation study
  for (i in seq_len(ncol(parameters_df))) {
    ...
    for (j in seq(n_params_loop)) {
      ...
      if (metric %in% c("tie", "success_proba")) {
        plt <- plt +
          ggplot2::geom_hline(
            yintercept = analysis_config$nominal_tie,
...
plot_metric_vs_sample_size <- function(metric,
  ...
  params_str <- convert_params_to_str(
    methods_dict[[method]],
    parameters_combinations
  )
  ...
  plt <- plt +
    ggplot2::labs(
      title = title, x = "Target study sample size per arm",
      y = selected_label
    )
...
plot_metric_vs_scenario <- function(results_metrics_df, metrics) {
  analysis_config <- yaml::read_yaml(system.file("conf/analysis_config.yml",
    package = "RBExT"
  ))
        ...
        if (!(metric %in% c("tie"))) {
          for (sample_size in target_sample_sizes) {
            for (i in seq_len(nrow(parameters_combinations))) {
              # Run plot_metric_vs_drift function with control_drift equal to
              # FALSE
              categories <- c(
              ...
              for (category in categories) {
                if (length(unique(filtered_results_metrics_df[[category]])) ==
                      1) {
                  next
                } else {
