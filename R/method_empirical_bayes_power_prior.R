...
#'
#' @description This function finds the calibration parameter for type I error
#'   control in empirical Bayes power prior methods.
#' It is based on the code in Nikolakopoulos et al, 2018, "Dynamic borrowing
#' through adaptive power priors that control type I error". We just renamed
#' the variables to be more explicit.
...
#' @param target_data_sampling_variance Sampling variance of the target study
#'   data
#' @param source_data_sampling_variance Sampling variance of the source study
#'   data
#' @param tolerance Tolerance for convergence of the estimation
#' @param seed Seed for random number generation
#' @param theta_0 True mean for type I error computation
#' @return A matrix containing the calibration parameter and other related
#'   values
#' @export
find_calibration_parameter <- function(n_iter = 1e6,
  ...
  n0 <- target_sample_size_per_arm * target_data_sampling_variance /
    source_data_sampling_variance

  std_predictive_dist <- sqrt(
    target_data_sampling_variance / n0 + target_data_sampling_variance /
      target_sample_size_per_arm
  )

  max_z_1_m_c2 <- -(((
    -(
      sqrt(target_data_sampling_variance) * sqrt(n0 +
                                                   target_sample_size_per_arm) * qnorm(significance_level) + n0 *
        source_treatment_effect_estimate
    )
  ) / (
  ...
  min_prior_mean <- -(
    ...
    qnorm(
          significance_level) # significance level was hard coded as 0.05 here in the original code
  ...
  upper_limit <- min(max_z_1_m_c2, 1)
  lower_limit <- 0
  z_1_m_c2 <- min(
    max_z_1_m_c2,
    1
  ) # Z_1-c/2 : as defined in the manuscript, calibration parameter for type I error control
  t1 <- ex_t1 <- pnorm((
    sqrt(target_data_sampling_variance) * sqrt(n0 +
                                                 target_sample_size_per_arm) *
  ...
  if (t1 < desired_tie) {
    ...
    while ((abs(t1 - t2) > tolerance) &&
      ...
      while ((t1 > desired_tie) &&
        ...
        t1 <- sum(((
          a1 * n0 * source_treatment_effect_estimate + x *
            target_sample_size_per_arm
      ...
      if (t1 < pnorm((
        sqrt(n0 + target_sample_size_per_arm) * qnorm(significance_level) +
          n0 * source_treatment_effect_estimate
      ) / sqrt(
...
#'
#' @description This is a parent class for variants of empirical Bayes PP
#'   methods for normally distributed summary measure of the
#'   treatment effect.
#'
...
gaussian_empirical_bayes_pp <- R6::R6Class(
  ...
  public = list(
    ...
    hypothesis_space_transformation = function(target_data) {
      # To handle the cases where null_space == "right", we apply the following
      # transformations to put ourselves back in a situation equivalent to
      # null_space == "left":
      ...
      if (self$null_space == "right") {
        ...
        stop(
             paste0("The null space must be either on the",
               " left side or on the right side of", " theta_0"))
      }
      theta_0 <- self$parameters$theta_0
      # To handle the cases where theta_0 != 0, we apply the following
      # translations to put ourselves back in a situation equivalent to theta_0
      # == 0
      source_treatment_effect_estimate <-
        source_treatment_effect_estimate - theta_0
      target_treatment_effect_estimate <-
        target_treatment_effect_estimate - theta_0
    ...
    empirical_bayes_update = function(target_data) {
      ...
      self$posterior_parameters$power_parameter <-
        self$power_parameter_estimation(target_data = target_data)

      self$prior_var <- (self$prior$source$standard_error^2) /
        self$posterior_parameters$power_parameter

      if (self$posterior_parameters$power_parameter != 0) {
        self$prior_var <- (self$prior$source$standard_error^2) /
          self$posterior_parameters$power_parameter # In the Gaussian case, the power prior is equivalent to having a Gaussian prior with variance prior_var
      } else {
    ...
    prior_pdf = function(target_treatment_effect) {
      if (is.null(self$prior_var)) {
        stop(
          paste0("Prior variance is not defined. Run",
            " model$empirical_bayes_update(target_data)",
              " befor calling model$prior_pdf().")
        )
      }
      dnorm(
...
#'
#' @description This class inherits from Gaussian_empirical_Bayes_PP and
#'   implements the Gravestock's EBPP method.
#'
#' @format R6Class object.
...
gaussian_gravestock_ebpp <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description Estimate the power parameter using the Gravestock's EBPP
    #'   method.
    #'
    #' @param target_data The target data.
    #' @param source_treatment_effect_estimate Treatment effect estimate in the
    #'   source study
    #' @param target_treatment_effect_estimate Treatment effect estimate in the
    #'   target study
    #' @return The estimated power parameter.
    power_parameter_estimation = function(target_data) {
      # We reuse the code from the PDCCPP, but leverage the fact that PDCCPP is
      # equivalent to Gravestock's EBPP with the calibration parameter set as:

      transformed_treatment_effects <-
        self$hypothesis_space_transformation(target_data = target_data)
      source_treatment_effect_estimate <-
        transformed_treatment_effects$source_treatment_effect_estimate
      target_treatment_effect_estimate <-
        transformed_treatment_effects$target_treatment_effect_estimate


      calibration_parameter <- 2 *
        (1 - pnorm(1)) # See Nikolakopoulos et al, 2018

      target_data_sampling_variance <-
        target_data$sample$treatment_effect_standard_error^
        2 * target_data$sample_size_per_arm

      # We compute the sampling variance for the corresponding Gaussian, the
      # goal is to put ourselves in a case equivalent to sampling from a
      # Gaussian (single arm)
      source_data_sampling_variance <- self$prior$source$standard_error^
        2 * self$prior$source$equivalent_source_sample_size_per_arm

      n0 <- target_data$sample_size_per_arm * target_data_sampling_variance /
        source_data_sampling_variance
      ...
      power_parameter <- ifelse(((
        target_treatment_effect_estimate > (
          source_treatment_effect_estimate + standard_deviation_predictive *
...
pdccpp <- R6::R6Class(
  ...
  public = list(
    ...
    #'
    #' @param target_data The target data.
    #' @param source_treatment_effect_estimate The source treatment effect
    #'   estimate.
    #' @param target_treatment_effect_estimate The target treatment effect
    #'   estimate.
    #' @return The estimated power parameter.
    power_parameter_estimation = function(target_data) {
      transformed_treatment_effects <-
      ...
      source_data_sampling_variance <- equivalent_source_sample_size_per_arm *
        self$prior$source$standard_error^2

      calibration <- find_calibration_parameter(
...
p_value_based_pp_gaussian <- R6::R6Class(
  ...
  public = list(
    ...
    #'
    #' @description This method estimates the power parameter for the given
    #'   target data.
    #'
    #' @param target_data The target data object.
    ...
    power_parameter_estimation = function(target_data) {
      ...
      power_parameter <- exp((self$parameters$shape_parameter /
                                (1 - p_value)) * log(1 - p_value))
      if (is.null(power_parameter) || is.na(power_parameter)) {
        stop("Variable is NULL or NA. Execution stopped.")
      }
      power_parameter
...
p_value_based_pp_binomial <- R6::R6Class(
  ...
  public = list(
    ...
    test = function(target_data,
      ...
      # # H0a : θS - θT > λ
      # z_left <- (
      # source_treatment_effect_estimate - target_treatment_effect_estimate -
      # self$parameters$equivalence_margin
      # ) / se
      ...
      left <- BSDA::zsum.test(
        ...
        mu = self$parameters$equivalence_margin,
        alternative = "less",
        sigma.x = self$prior$source$standard_error *
