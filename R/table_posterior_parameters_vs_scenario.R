...
#'
#' This function creates tables comparing posterior parameters to drift for a
#' given method, case study, and sample size.
#'
...
#' @param target_sample_size_per_arm The target sample size per arm to filter
#'   the results by.
...
#'
#' @return This function does not return a value. It creates and saves tables
#'   in HTML, PDF, and LaTeX formats.
#'
#' @import dplyr
#' @import knitr
#' @export
table_posterior_parameters_vs_drift <- function(results_metrics_df,
  ...
  posterior_parameters_df[] <- lapply(posterior_parameters_df, format_num,
    digits = 4, scientific = TRUE
  )
  # Identify columns with unique values and drop them
  cols_to_keep <- sapply(
    posterior_parameters_df,
    function(col) length(unique(col)) > 1
  )
  posterior_parameters_df <- posterior_parameters_df[, cols_to_keep,
    drop =
      FALSE
  ]
  posterior_parameters_df <- convert_ci_columns(posterior_parameters_df)

  if (nrow(posterior_parameters_df) == 0 || ncol(posterior_parameters_df) ==
        0) {
    return()
  }
  ...
  unique_prior_params <- prior_params[sapply(
    prior_parameters_df,
    function(col) length(unique(col)) == 1
  )]
  updated_prior_parameters_df <- prior_parameters_df[, cols_to_keep,
    drop =
      FALSE
  ]
  ...
  rename_parameters_dplyr <- function(df, prefix) {
    df |>
      rename_with(
        .cols = any_of(keys),
        .fn = ~ sprintf("%s %s", prefix, sapply(
          .x,
          function(key) methods_dict[[method]][[key]]$parameter_notation
        ))
      )
  }

  # Rename columns in prior_parameters_df
  updated_prior_parameters_df <-
    rename_parameters_dplyr(updated_prior_parameters_df, "Prior")
  ...
  rename_posterior_parameters_dplyr <- function(df) {
    df |>
      rename_with(
        .cols = everything(),
        # Apply to all columns, or replace `everything()` with specific columns
  ...
  posterior_parameters_df <-
    rename_posterior_parameters_dplyr(posterior_parameters_df)

  data_table <- cbind(
    results_metrics_df$drift, updated_prior_parameters_df,
    posterior_parameters_df
  )
  ...
  for (key in unique_prior_params) {
    ...
    prior_params <- paste0(prior_params, sprintf(
      "%s %s = %s", "Prior",
      methods_dict[[method]][[key]]$parameter_notation,
      unique(prior_parameters_df[key])
    ))
  }
  ...
  if (method ==
        "commensurate_power_prior") { # TODO: replace this hard-coded part
    ...
    if (is.character(x)) {
      sapply(x, function(y) {
        if (!is.na(y) && !grepl(
          "^\\s*NA\\s*$",
          y
        )) { # Ignore actual NA and "NA" strings
          str_to_title(trimws(
                              y)) # Capitalize words and remove any leading/trailing spaces
        } else {
...
#'
#' This function generates multiple tables for posterior parameters across
#' different case studies, methods, and sample sizes.
#'
#' @param results_metrics_df A data frame containing the results metrics.
#' @param metrics A list or vector of metrics to include in the analysis.
#'
#' @return This function does not return a value. It calls other functions to
