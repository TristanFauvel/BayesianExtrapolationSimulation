#' Calculate the moment-based Effective Sample Size (ESS) for a Gaussian
#' mixture.
#'
#' @description This function calculates the moment-based ESS for a Gaussian
#'   mixture distribution.
#' It takes a mixture object and an optional prior reference scale as input and
#' returns the ESS.
#'
...
#' Calculate the precision-based Effective Sample Size (ESS) for a Gaussian
#' mixture.
#'
#' @description This function calculates the precision-based ESS for a Gaussian
#'   mixture distribution.
#' It takes a mixture object and an optional prior reference scale as input and
#' returns the ESS.
#'
...
gaussian_mix_precision_ess <- function(mix) {
  # Precision-based matching method for ESS estimation, for Gaussian mixtures
  # only.
  sigma <- RBesT::sigma(mix)
  if (is.null(sigma)) {
  ...
  index_97_5 <- grep("97.5%", names(smix))
  index_2_5 <- grep("2.5%", names(smix))
  # Calculate half-width of 95% confidence interval
  half_width <- as.numeric((smix[index_97_5] - smix[index_2_5]) / 2)
...
#' Calculate the prior moment-based Effective Sample Size (ESS) for a Bayesian
#' model.
#'
#' @description This function calculates the prior moment-based ESS for a
#'   Bayesian model using the RBesT package.
#' It takes a RBesT model object and target data as input and returns the prior
#' moment-based ESS.
#'
...
prior_moment_ess <- function(rbest_model, target_data) {
  ...
  posterior_moment_ess - target_data$sample_size_per_arm
}

#' Calculate the prior precision-based Effective Sample Size (ESS) for a
#' Bayesian model.
#'
#' @description This function calculates the prior precision-based ESS for a
#'   Bayesian model using the RBesT package.
#' It takes a RBesT model object and target data as input and returns the prior
#' precision-based ESS.
#'
...
prior_precision_ess <- function(rbest_model, target_data) {
  ...
  posterior_precision_ess - target_data$sample_size_per_arm
}


#' Calculate the priorEffective Sample Size (ESS) based on ELIR for a Bayesian
#' model.
#'
#' @description This function calculates the ELIR for a Bayesian model using
#'   the RBesT package.
#' It takes a RBesT model object and target data as input and returns the prior
#' moment-based ESS.
#'
...
prior_ess_elir <- function(rbest_model, target_data) {
  ...
  tryCatch(
    {
      elir <- RBesT::ess(rbest_model,
        method = "elir", sigma =
          RBesT::sigma(rbest_model)
      )
      return(elir)
    },
    error = function(e) {
