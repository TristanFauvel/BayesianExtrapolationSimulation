plot_element_metric_vs_xvar_methods <- function(plt,
  ...
  plt <- plt + labs(color = labels_title) + scale_color_viridis_d(
    labels =
...
plot_metric_vs_sample_size_methods <- function(metric,
  ...
  results_df <- results_df |>
    ...
    mutate(
      jitter_offset = jitter(0, amount = 4)[1],
      # Generate a single jitter offset per group
      jittered_x = target_sample_size_per_arm +
        jitter_offset # Add the same offset to all rows in the group
    ) |>
    ungroup()
  ...
  plt <- plt + purrr::imap(results_df_split, function(df_subset, method_name) {
    ...
    layers <- list(
      ...
      geom_point(
        data = df_subset,
        aes(x = jittered_x, color = factor(parameters_labels,
          levels =
            method_parameters
        )),
        shape = method_shape,
    ...
    for (metric in names(metrics)) {
      if (!(metric %in% c("tie"))) {
        for (source_denominator_change_factor in unique(
                                                        results_metrics_df1$source_denominator_change_factor)) {
          ...
          for (target_to_source_std_ratio in unique(
                                                    results_metrics_df2$target_to_source_std_ratio)) {
