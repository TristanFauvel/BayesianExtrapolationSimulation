...
forest_subplot <- function(data,
  ...
  # scale_x_continuous(
  # breaks = pretty(data[[x_metric_name]], n = 5), # Set 10 evenly spaced
  # breaks
  # labels = function(x) format(x, nsmall = 2) # Optional: Format to 2 decimal
  # places
  # )
  if (!ylabel) {
    plt <- plt + theme(
  ...
  if (x_metric_name == "posterior_mean" ||
        x_metric_name == "posterior_median" || x_metric_name ==
        "point_estimate_mean") {
    theta_0 <- unique(data$theta_0)
    if (legend) {
      plt <- plt + geom_vline(aes(xintercept = theta_0, color = "theta_0"),
        linetype = "dotted"
      ) +
        scale_color_manual(
          name = "",
    ...
    if (is.null(data$equivalent_source_sample_size_per_arm) ||
      ...
      equivalent_source_sample_size_per_arm <- 2 *
    ...
    source_sample_size_per_arm <-
      unique(round(equivalent_source_sample_size_per_arm))

    if (legend) {
      plt <- plt +
        geom_vline(
          aes(
            xintercept = target_sample_size_per_arm, color =
        ...
        geom_vline(
          aes(
            xintercept = source_sample_size_per_arm, color =
              "Source sample size per arm"
          ),
          linetype = "dotted"
        ) +
        scale_color_manual(
...
forest_combined_plot <- function(data,
  ...
  if (x_metric_name == "posterior_mean" || x_metric_name ==
        "posterior_median" || x_metric_name == "point_estimate_mean") {
    theta_0 <- unique(data$theta_0)
    plt <- plt + geom_vline(
      xintercept = theta_0, linetype = "dotted", color =
        "black"
    )
  } else if (x_metric_name %in% c("ess_moment", "ess_precision", "ess_elir")) {
    target_sample_size_per_arm <- unique(data$target_sample_size_per_arm)
    equivalent_source_sample_size_per_arm <- ifelse(
      is.null(data$equivalent_source_sample_size_per_arm) |
        any(is.na(data$equivalent_source_sample_size_per_arm)),
      2 * data$source_sample_size_control *
        data$source_sample_size_treatment / (data$source_sample_size_control +
                                               data$source_sample_size_treatment),
      data$equivalent_source_sample_size_per_arm
    )
    source_sample_size_per_arm <-
      unique(round(equivalent_source_sample_size_per_arm))
    ...
    plt <- plt +
      geom_vline(
        xintercept = target_sample_size_per_arm, linetype = "dotted",
        color = "black"
      ) +
      geom_vline(
        xintercept = source_sample_size_per_arm, linetype = "dotted",
        color = "red"
      )
  }
...
#'
#' @description This function generates a forest plot based on the provided
#'   results dataframe, selected case study, and selected
#'   target sample size per arm.
#'
#' @param results_freq_df The results dataframe.
#' @param selected_case_study The selected case study.
#' @param selected_target_sample_size_per_arm The selected target sample size
#'   per arm.
...
forest_plot <- function(results_freq_df, x_metric, panels = TRUE) {
  ...
  selected_target_sample_size_per_arm <-
    unique(results_freq_df$target_sample_size_per_arm)
  ...
  x_metric_uncertainty_lower <- rlang::sym(paste0(
    metric$metric_uncertainty,
    "_lower"
  ))
  x_metric_uncertainty_upper <- rlang::sym(paste0(
    metric$metric_uncertainty,
    "_upper"
  ))
  ...
  target_to_source_std_ratio <-
    unique(results_freq_df$target_to_source_std_ratio)
  source_denominator_change_factor <-
    unique(results_freq_df$source_denominator_change_factor)
  ...
  # The following is to handle problems with the way R handles floating point
  # precision.
  ...
  df <- df |>
    ...
    filter(any(sapply(
      closest_values,
      function(val) is_approx_equal(target_treatment_effect, val)
    ))) |>
    ungroup() |>
  ...
  df <- df |>
    mutate(target_treatment_effect = sapply(target_treatment_effect,
      closest_value,
  ...
  no_effect_data <- df[df[, "target_treatment_effect"] ==
                         "No treatment effect", ]
  partially_consistent_effect_data <- df[df[, "target_treatment_effect"] ==
                                           "Partially consistent treatment effect", ]
  consistent_effect_data <- df[df[, "target_treatment_effect"] ==
                                 "Consistent treatment effect", ]

  if (nrow(no_effect_data) != nrow(partially_consistent_effect_data) ||
        nrow(consistent_effect_data) !=
          nrow(partially_consistent_effect_data) ||
        nrow(no_effect_data) != nrow(consistent_effect_data)) {
    warning(
            paste0("Inconsistency in the number of rows in",
              " the partially consistent, consistent,",
                " and no-effect data. Skipping this plot."))
    return()
  }
  ...
  partially_consistent_effect_data <-
    partially_consistent_effect_data[new_order, ]
  ...
  if (panels == TRUE) {
    ...
    grob_partially_consistent_effect <-
      ggplotGrob(plot_partially_consistent_effect)
    grob_consistent_effect <- ggplotGrob(plot_consistent_effect)

    grid::grid.newpage()

    # Note that adding the Tikz export to the export_plots function does not
    # work.
    ...
    combined_data <- rbind(
      ...
      cbind(partially_consistent_effect_data,
        effect_type =
          "Partially consistent"
      ),
...
forest_plot_bayesian <- function(results_bayes_df, x_metric) {
  ...
  selected_target_sample_size_per_arm <-
    unique(results_bayes_df$target_sample_size_per_arm)
  ...
  analysis_design_prior_data <- df[df[, "design_prior_type"] ==
                                     "analysis_prior", ]
  source_posterior_design_prior_data <- df[df[, "design_prior_type"] ==
                                             "source_posterior", ]
  ...
  source_posterior_design_prior_data <-
    source_posterior_design_prior_data[new_order, ]
  ...
  grob_source_posterior_design_prior <-
    ggplotGrob(plot_source_posterior_design_prior)
...
#'
#' @description This function plots the comparison of methods based on the
#'   provided results metrics dataframe.
#'
...
forest_plot_methods_comparison <- function(results_metrics_df, metrics) {
  ...
  results_metrics_df <- results_metrics_df[(results_metrics_df[
    ,
    "source_denominator_change_factor"
  ] == 1 &
    !is.na(results_metrics_df[
      ,
      "source_denominator_change_factor"
    ])), ]
  ...
  for (metric in names(metrics)) {
    ...
    for (case_study in case_studies) {
      results_metrics_df1 <-
        results_metrics_df[results_metrics_df$case_study == case_study, ]
      ...
      for (target_sample_size_per_arm in target_sample_sizes) {
        results_metrics_df2 <-
          results_metrics_df1[
                              results_metrics_df1$target_sample_size_per_arm ==
...
#'
#' @return None
forest_plot_methods_comparison_bayesian <- function(
  results_metrics_df,
  metrics
) {
  case_studies <- unique(results_metrics_df$case_study)
