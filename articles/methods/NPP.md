...
Normalized Power Prior in the case where the treatment effect follows a
normal distribution.

The derivation for the normalized power prior can be found in [Pawel et
al (2023)](https://doi.org/10.1002/sta4.591)

With known standard error $`\sigma_T`$ of the estimate $`\theta_T`$, a
Beta prior on the power parameter
$`\gamma \sim \operatorname{Be}(p, q)`$. This choice leads to the
...
Combining this prior with the likelihood of the target study data
produces a joint posterior for $`\theta`$ and $`\gamma`$, that is,
``` math
...
```
from which a marginal posterior for $`\gamma`$ can be obtained by
integrating out $`\theta`$, that is,
``` math
...
We reuse the code published in [Pawel et al
(2023)](https://doi.org/10.1002/sta4.591). Marginal posterior densities
...
![](NPP_files/figure-html/unnamed-chunk-5-1.png)
...
![](NPP_files/figure-html/unnamed-chunk-6-1.png) \## Power parameter as
a function of drift
...
![](NPP_files/figure-html/unnamed-chunk-8-1.png)
...
![](NPP_files/figure-html/unnamed-chunk-9-1.png)
